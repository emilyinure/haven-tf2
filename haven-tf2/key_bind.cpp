...
auto c_key_bind::paint( ) -> void {
	...
	// set our item area.
	this->item_area_ = { this->m_area.m_x + ( this->m_area.m_w - ( this->m_area.m_w * 0.5f ) ), this->m_area.m_y + 2, this->m_area.m_w * 0.5f, 4 * 13.f };
	...
	// current value.
	if ( !this->open_ )
		g_ui.m_draw.text( g_render.m_fonts.menu.main, { this->dropdown_area_.m_x + 2, this->dropdown_area_.m_y }, g_input.key( this->value_->key ), color( 255, 255, 255, 100 * g_ui.m_fade ), e_text_alignment::text_align_left );
	...
	for ( auto i = 0; i < 4; i++ ) {
		...
		g_ui.m_draw.text( g_ui.m_draw.fonts.main, { this->item_area_.m_x + 2, this->item_area_.m_y + ( i * 13 ) }, item, this->value_->type == i ? g_ui.m_theme : color( 255, 255, 255, ( hovering ? 150 : 100 ) * g_ui.m_fade ), e_text_alignment::text_align_left );
	}
	g_ui.m_theme.m_a = 255;
}
