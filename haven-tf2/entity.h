...
enum e_life_state { life_alive, life_dying, life_dead, life_respawnable, life_discardbody };
...
public:
  ...
  matrix_3x4*    BoneArray() { return (matrix_3x4*)((char*)(this + 1) + m_matrixOffset); }
  short* CachedToStudio() { return (short*)((char*)(this + 1) + m_cachedToStudioOffset); }
...
class c_base_entity : public i_client_entity {
  ...
  enum e_indexes { GET_HEALTH = 106, GET_MAX_HEALTH = 107 };
...
public:
  ...
  float   sim_time();
  float   spawn_time();
  int     collision_group() { return get<int>(896); }
  bool    should_collide(int group, int mask) {
    return g_utils.get_virtual_function<bool (*)(void*, int, int)>(this, 145)(this, group,
                                                                              mask);
  }
...
enum EWeaponSlots : int { SLOT_PRIMARY, SLOT_SECONDARY, SLOT_MELEE };
...
public:
  c_utl_vector<matrix_3x4>* GetCachedBoneData() {
    return reinterpret_cast<c_utl_vector<matrix_3x4>*>(reinterpret_cast<DWORD>(this) +
                                                       0x848); // 0x844?
  }
  c_tf_player_shared* m_shared();
  bool                in_cond(e_tf_cond cond) { return m_shared()->in_cond(cond); }
  ...
  int&           hitbox_set();
  float&         model_scale();
  vector         get_hitbox_pos(int iHitbox, matrix_3x4* matrix = nullptr,
                                vector mins = vector(0.5f, 0.5f, 0.5f),
                                vector max  = vector(0.5f, 0.5f, 0.5f));

  move_type_t move_type();

  bool is_alive();
  bool is_valid(c_base_player* local, bool team_check = true, bool dormant_check = true,
                bool alive_check = true);
};
