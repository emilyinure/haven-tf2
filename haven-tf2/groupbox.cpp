...
void c_groupbox::paint() {
  ...
  // title outline.
  // g_ui.m_draw.outlined_rect( { this->m_area.m_x, this->m_area.m_y, this->m_area.m_w, 15 }, {
  // 65, 65, 65 } );

  // title text.
  g_render.text(g_render.m_fonts.menu.main,
                {this->m_area.m_x + (this->m_area.m_w * 0.5f), this->m_area.m_y}, this->m_name,
                color(255, 255, 255, 100 * g_ui.m_fade), e_text_alignment::text_align_center);

  this->paint_children();
}

void c_groupbox::update() { this->update_children(); }
...
std::shared_ptr<c_key_bind> c_groupbox::add_keybind(const char* name, const char* tooltip,
                                                    int default_type) {
...
std::shared_ptr<c_button>
c_groupbox::add_button(const char* name, c_button::callback_fn callback, const char* tooltip) {
...
std::shared_ptr<c_slider> c_groupbox::add_slider(const char* name, float min, float max,
                                                 const char* tooltip) {
