...
public:
  virtual bool  precache_sound(const char* sample, bool preload = false,
                               bool is_ui_sound = false) = 0;
  ...
  virtual void  emit_sound(i_recipient_filter& filter, int ent_index, int channel,
                           const char* sample, float fl_volume, float fl_attenuation,
                           int flags = 0, int pitch = PITCH_NORM, int special_dsp = 0,
                           const vector* origin = NULL, const vector* direction = NULL,
                           c_utl_vector<vector>* utl_vec_origins = NULL,
                           bool update_positions = true, float soundtime = 0.0f,
                           int speakerentity = -1)       = 0;
  virtual void  emit_sound(i_recipient_filter& filter, int ent_index, int channel,
                           const char* sample, float fl_volume, sound_level_t sound_level,
                           int flags = 0, int pitch = PITCH_NORM, int special_dsp = 0,
                           const vector* origin = NULL, const vector* direction = NULL,
                           c_utl_vector<vector>* utl_vec_origins = NULL,
                           bool update_positions = true, float soundtime = 0.0f,
                           int speakerentity = -1)       = 0;
  virtual void  emit_sentence_by_index(
       i_recipient_filter& filter, int ent_index, int channel, int sentence_index,
       float fl_volume, sound_level_t sound_level, int flags = 0, int pitch = PITCH_NORM,
       int special_dsp = 0, const vector* origin = NULL, const vector* direction = NULL,
       c_utl_vector<vector>* utl_vec_origins = NULL, bool update_positions = true,
  ...
  virtual void  emit_ambient_sound(const char* sample, float fl_volume, int pitch = PITCH_NORM,
                                   int flags = 0, float soundtime = 0.0f)                 = 0;
