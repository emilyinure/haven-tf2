...
void c_tab::paint( ) {
	...
	// title.
	g_ui.m_draw.text( g_render.m_fonts.menu.tabs, { this->m_area.m_x + ( this->m_area.m_w * 0.5f ) , this->m_area.m_y + 2 }, this->m_name, color( 255, 255, 255, 100 * g_ui.m_fade ), e_text_alignment::text_align_center );

	// bars.
	//g_ui.m_draw.outlined_rect( { this->m_area.m_x, this->m_area.m_y + this->m_area.m_h - 3, this->m_area.m_w, 1 }, this->m_enabled ? g_ui.m_theme : color( 35, 35, 35, 30 ) );
	g_ui.m_theme.m_a = 100 * g_ui.m_fade;
	if( this->m_enabled )
		g_ui.m_draw.filled_rect( { this->m_area.m_x, this->m_area.m_y + this->m_area.m_h - 2, this->m_area.m_w, 1 }, g_ui.m_theme - 20 );
...
void c_tab::update_children( ) {
	...
	for ( auto i = 0; i < this->m_groupboxes.size( ); i++ ) {
		...
		// handle position.
		child->m_area = {
			child->m_render_area.m_x + this->m_item_area.m_x,
			child->m_render_area.m_y + this->m_item_area.m_y,
			child->m_render_area.m_w,
