...
bool CTraceFilterSimple::should_hit_entity( c_base_entity* pHandleEntity, int contentsMask ) {
	if ( !oPassServerEntityFilter )
		oPassServerEntityFilter = g_modules.get( "client.dll" ).get_sig( "55 8B EC 56 8B 75 ? 85 F6 75 ? B0 ?" ).as< PassServer >( );
	if ( !oShouldCollide )
		oShouldCollide = g_modules.get( "client.dll" ).get_sig( "55 8B EC 8B 55 ? 56 8B 75 ? 3B F2" ).as< ShouldCollide >( );
	if ( !oStandardFilterRules )
		oStandardFilterRules = g_modules.get( "client.dll" ).get_sig( "55 8B EC 8B 4D ? 56 8B 01 FF 50 ? 8B F0 85 F6 75 ? B0 ? 5E 5D C3" ).as< StandardFilter >( );
	...
	"55 8B EC 8B 4D ? 56 8B 01 FF 50 ? 8B F0 85 F6 75 ? B0 ? 5E 5D C3";//bool __cdecl StandardFilterRules(int a1, int a2)
