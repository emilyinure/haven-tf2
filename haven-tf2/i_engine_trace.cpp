  ...
  if (!oShouldHitEntity)
    oShouldHitEntity =
        g_modules.get("client.dll")
            .get_sig("48 89 5C 24 08 48 89 6C 24 10 48 89 74 24 18 57 41 56 41 57 48 83 EC 20 "
                     "48 8B 02 48 8B E9") // first index of CTraceFilterSimple
            .as<ShouldHitEntity>();
  return oShouldHitEntity(this, pHandleEntity, contentsMask);
}

bool CTraceFilterIgnorePlayers::should_hit_entity(c_base_entity* pServerEntity,
                                                  int            contentsMask) {
  if (!(pServerEntity->entindex() == 0 ||
        pServerEntity->entindex() >= g_interfaces.m_engine->get_max_clients()))
    return false;

  return CTraceFilterSimple::should_hit_entity(pServerEntity, contentsMask);
}

bool CTraceFilterIgnoreTeammates::should_hit_entity(c_base_entity* pServerEntity,
                                                    int            contentsMask) {
  if (!(pServerEntity->entindex() == 0 ||
        pServerEntity->entindex() >= g_interfaces.m_engine->get_max_clients())) {
