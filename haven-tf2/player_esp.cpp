	...
	for (auto i = 1; i < g_interfaces.m_global_vars->m_max_clients; i++) {
		...
		if (player_esp.name->m_value)
			if (g_interfaces.m_engine->get_player_info(player->entindex(), &player_info))
				g_render.text(g_render.m_fonts.menu.main, { bounding_box.m_x + (bounding_box.m_w * 0.5f), bounding_box.m_y - 15 }, player_info.m_name, g_cl.m_local->m_i_team_num() == player->m_i_team_num() ? g_ui.m_theme : color(255, 255, 255), text_align_center);

		if (player_esp.box->m_value) {
			g_render.outlined_rect({ bounding_box.m_x - 1, bounding_box.m_y - 1, bounding_box.m_w + 2, bounding_box.m_h + 2 }, { 0, 0, 0 });
			g_render.outlined_rect({ bounding_box.m_x + 1, bounding_box.m_y + 1, bounding_box.m_w - 2, bounding_box.m_h - 2 }, { 0, 0, 0 });
		...
		if (player_esp.flags->m_value) {
			...
			auto draw_flag = [&flag_offset, &bounding_box, player](const char* flag_text, const color& flag_color) mutable {
				const color col = { 255, 255, 255 };

				const vector_2d flag_text_size = g_render.get_text_size(flag_text, g_render.m_fonts.menu.main);
				g_render.text(g_render.m_fonts.menu.main, { (bounding_box.m_x + bounding_box.m_w) + 4, (bounding_box.m_y + flag_offset) - 2 }, flag_text, col);
		...
		if (player_esp.weapon->m_value) {
			...
			g_render.text(g_render.m_fonts.menu.main, { bounding_box.m_x + (bounding_box.m_w * 0.5f), bounding_box.m_y - 30 }, weapon_text.c_str(), {255, 255, 255, 255}, text_align_center);
		}
	}
