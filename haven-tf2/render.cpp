...
void c_render::init() {
  ...
  this->m_fonts.menu.title =
      create_font("Segoe UI", 13, 200, e_font_flags::font_flag_antialias);
  this->m_fonts.menu.tabs =
      create_font("Segoe UI", 18, 200,
                  e_font_flags::font_flag_antialias | e_font_flags::font_flag_dropshadow);
  this->m_fonts.menu.tooltip =
      create_font("Tahoma", 13, 300, e_font_flags::font_flag_dropshadow);
}

void c_render::filled_rect(const box_t box, const color col) {
  g_interfaces.m_surface->draw_set_color(col);
  g_interfaces.m_surface->draw_filled_rect(box.m_x, box.m_y, box.m_x + box.m_w,
                                           box.m_y + box.m_h);
}
...
void c_render::outlined_rect(const box_t box, const color col) {
  g_interfaces.m_surface->draw_set_color(col);
  g_interfaces.m_surface->draw_outlined_rect(box.m_x, box.m_y,
                                             static_cast<int>(box.m_x + box.m_w),
...
void c_render::box(const vector from, const vector to, const color col) {
  ...
  vector from_screen, to_screen, from_right_screen, from_forward_screen, from_up_screen,
      to_forward_screen, to_right_screen, to_down_screen;

  if (!math::world_to_screen(from, from_screen) || !math::world_to_screen(to, to_screen) ||
  ...
  g_interfaces.m_surface->draw_set_color(col);
  g_interfaces.m_surface->draw_line(from_screen.m_x, from_screen.m_y, from_right_screen.m_x,
                                    from_right_screen.m_y);
  g_interfaces.m_surface->draw_line(from_screen.m_x, from_screen.m_y, from_forward_screen.m_x,
                                    from_forward_screen.m_y);
  g_interfaces.m_surface->draw_line(from_screen.m_x, from_screen.m_y, from_up_screen.m_x,
                                    from_up_screen.m_y);

  g_interfaces.m_surface->draw_line(to_screen.m_x, to_screen.m_y, to_right_screen.m_x,
                                    to_right_screen.m_y);
  g_interfaces.m_surface->draw_line(to_screen.m_x, to_screen.m_y, to_forward_screen.m_x,
                                    to_forward_screen.m_y);
  g_interfaces.m_surface->draw_line(to_screen.m_x, to_screen.m_y, to_down_screen.m_x,
                                    to_down_screen.m_y);

  g_interfaces.m_surface->draw_line(to_down_screen.m_x, to_down_screen.m_y,
                                    from_right_screen.m_x, from_right_screen.m_y);
  g_interfaces.m_surface->draw_line(to_down_screen.m_x, to_down_screen.m_y,
                                    from_forward_screen.m_x, from_forward_screen.m_y);

  g_interfaces.m_surface->draw_line(from_up_screen.m_x, from_up_screen.m_y, to_right_screen.m_x,
                                    to_right_screen.m_y);
  g_interfaces.m_surface->draw_line(from_up_screen.m_x, from_up_screen.m_y,
                                    to_forward_screen.m_x, to_forward_screen.m_y);

  g_interfaces.m_surface->draw_line(from_forward_screen.m_x, from_forward_screen.m_y,
                                    to_forward_screen.m_x, to_forward_screen.m_y);
  g_interfaces.m_surface->draw_line(from_right_screen.m_x, from_right_screen.m_y,
                                    to_right_screen.m_x, to_right_screen.m_y);
}
...
void c_render::render_verts(int count, Vertex_t* vertexes, color c) {
  static int Texture = g_interfaces.m_surface->create_texture_id(
      true);                                      // need to make a texture with procedural true
  unsigned char buffer[4] = {255, 255, 255, 255}; // r,g,b,a

  g_interfaces.m_surface->draw_set_texture_rgba(
      Texture, buffer, 1, 1, false,
      false); // Texture, char array of texture, width, height
  g_interfaces.m_surface->draw_set_color(
      c); // keep this full color and opacity use the RGBA @top to set values.
