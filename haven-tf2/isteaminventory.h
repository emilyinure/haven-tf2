...
enum ESteamItemFlags {
  ...
  k_ESteamItemNoTrade =
      1 << 0, // This item is account-locked and cannot be traded or given away.

  // Action confirmation flags - these flags are set one time only, as part of a result set
  k_ESteamItemRemoved =
      1 << 8, // The item has been destroyed, traded away, expired, or otherwise invalidated
  k_ESteamItemConsumed =
      1 << 9, // The item quantity has been decreased by 1 via ConsumeItem API.
...
public:
  ...
  virtual bool
  GetResultItemProperty(SteamInventoryResult_t resultHandle, uint32 unItemIndex,
  ...
  virtual bool CheckResultSteamID(SteamInventoryResult_t resultHandle,
                                  CSteamID               steamIDExpected) = 0;
  ...
  virtual bool GetItemsByID(SteamInventoryResult_t* pResultHandle,
                            STEAM_ARRAY_COUNT(unCountInstanceIDs)
  ...
  virtual bool DeserializeResult(SteamInventoryResult_t* pOutResultHandle,
                                 STEAM_BUFFER_COUNT(punOutBufferSize) const void* pBuffer,
                                 uint32 unBufferSize, bool bRESERVED_MUST_BE_FALSE = false) = 0;
  ...
  virtual bool GenerateItems(SteamInventoryResult_t* pResultHandle,
                             STEAM_ARRAY_COUNT(unArrayLength)
  ...
  virtual bool ConsumeItem(SteamInventoryResult_t* pResultHandle,
                           SteamItemInstanceID_t itemConsume, uint32 unQuantity) = 0;
  ...
  // exchange will fail.
  virtual bool
  ExchangeItems(SteamInventoryResult_t*                                        pResultHandle,
                ...
                STEAM_ARRAY_COUNT(unArrayDestroyLength)
  ...
  virtual bool TransferItemQuantity(SteamInventoryResult_t* pResultHandle,
                                    SteamItemInstanceID_t itemIdSource, uint32 unQuantity,
                                    SteamItemInstanceID_t itemIdDest) = 0;
  ...
  virtual bool TriggerItemDrop(SteamInventoryResult_t* pResultHandle,
  ...
  virtual bool TradeItems(SteamInventoryResult_t* pResultHandle, CSteamID steamIDTradePartner,
                          STEAM_ARRAY_COUNT(nArrayGiveLength)
                          ...
                          STEAM_ARRAY_COUNT(nArrayGetLength)
  ...
  virtual bool GetItemDefinitionIDs(
      STEAM_OUT_ARRAY_COUNT(punItemDefIDsArraySize, List of item definition IDs)
  ...
  // results may be copied.
  virtual bool
  GetItemDefinitionProperty(SteamItemDef_t iDefinition, const char* pchPropertyName,
                            STEAM_OUT_STRING_COUNT(punValueBufferSizeOut) char* pchValueBuffer,
                            uint32* punValueBufferSizeOut) = 0;
  ...
  // Starts the purchase process for the given item definitions.  The callback
  // SteamInventoryStartPurchaseResult_t will be posted if Steam was able to initialize the
  // transaction.
  //
  // Once the purchase has been authorized and completed by the user, the callback
  // SteamInventoryResultReady_t will be posted.
  STEAM_CALL_RESULT(SteamInventoryStartPurchaseResult_t)
  virtual SteamAPICall_t StartPurchase(STEAM_ARRAY_COUNT(unArrayLength)
                                           const SteamItemDef_t* pArrayItemDefs,
                                       STEAM_ARRAY_COUNT(unArrayLength)
  ...
  // Need to call RequestPrices() first.
        ...
        virtual bool GetItemPrice(SteamItemDef_t iDefinition, uint64* pCurrentPrice,
                                  uint64* pBasePrice) = 0;
        ...
        virtual bool                         RemoveProperty(SteamInventoryUpdateHandle_t handle,
        ...
        virtual bool SetProperty(SteamInventoryUpdateHandle_t handle,
                                 SteamItemInstanceID_t nItemID, const char* pchPropertyName,
                                 const char* pchPropertyValue) = 0;
        ...
        virtual bool InspectItem(SteamInventoryResult_t* pResultHandle,
                                 const char*             pchItemToken) = 0;
};
...
STEAM_DEFINE_USER_INTERFACE_ACCESSOR(ISteamInventory*, SteamInventory,
