...
public:
	...
	CTraceFilterCollisionArrows( c_base_entity* passentity )
	...
	virtual bool should_hit_entity( c_base_entity* pHandleEntity, int contentsMask ) {
		if ( pHandleEntity ) {
			...
			return ( pHandleEntity->entindex( ) == 0 || pHandleEntity->entindex( ) > g_interfaces.m_engine->get_max_clients( ) );
		}
...
public:
	virtual bool should_hit_entity( c_base_entity* pEntityHandle, int contentsMask ) {
		return ( pEntityHandle->entindex( ) == 0 || pEntityHandle->entindex( ) > g_interfaces.m_engine->get_max_clients( ) ) || ( pEntityHandle->m_i_team_num() != team );
	}
...
float get_speed ( ) {
	...
	case WPN_IronBomber:
		...
		//Charge = ( BeginCharge == 0.0f ) ? 0.0f : TICKS_TO_TIME( g_cl.m_local->m_tick_base( ) + 1 ) - BeginCharge;
		//weapon_speed = RemapValClamped( Charge,
		//	0.0f,
		//	4.f,
		//	900,
...
bool proj_aim::is_pipe( ) {
	...
	if ( item_id == WPN_IronBomber ||
		item_id == WPN_StickyLauncher ||
		item_id == WPN_LoooseCannon ||
		item_id == WPN_LochNLoad ||
		item_id == WPN_GrenadeLauncher ||
		item_id == WPN_NewGrenadeLauncher ||
...
void proj_aim::run( ) {
	...
	bool shoud_shoot = ( ticks_since_shot < 3) || ( g_cl.m_cmd->buttons_ & attack ) || g_ui.m_controls.aim.players.fire_mode->m_selected_index == 2;
...
void proj_aim::find_shot( bool& was_shoot, int attack ) {
	...
	float lerp = std::fmaxf( cl_interp->m_value.m_float_value, cl_interp_ratio->m_value.m_float_value / cl_updaterate->m_value.m_float_value );
	...
	for ( int steps = 0; steps < maxsteps; steps++ ) {
		...
		if ( g_cl.m_weapon->item_index( ) == WPN_StickyLauncher && ( ( steps * g_interfaces.m_global_vars->m_interval_per_tick ) < 0.8f ) )
			continue;
...
void proj_aim::draw( ) const {
	...
	for ( auto i = 1; i < m_path.size( ); i++ ) {
		...
		if ( g_interfaces.m_debug_overlay->screen_position( m_path[ i - 1 ], screen_1 ) || g_interfaces.m_debug_overlay->screen_position( m_path[ i ], screen_2 ) ) {
			continue;
		}
...
bool proj_aim::proj_can_hit( c_base_player* target, vector view, float goal_time, vector weapon_pos, bool record ) {
	...
	while ( time < goal_time - g_interfaces.m_global_vars->m_interval_per_tick ) {
		...
		ray_t ray;
		ray.initialize( pos, pos + ( velocity * g_interfaces.m_global_vars->m_interval_per_tick ) );//, bounds*-1, bounds );
		trace_t trace;
