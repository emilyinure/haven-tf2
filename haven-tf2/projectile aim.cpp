...
public:
  ...
  [[nodiscard]] virtual TraceType_t get_type() const { return TraceType_t::TRACE_EVERYTHING; }
};
  ...
  virtual bool should_hit_entity(c_base_entity* pHandleEntity, int contentsMask) {
    if (pHandleEntity) {
      ...
      if (pHandleEntity->collision_group() == TFCOLLISION_GROUP_ROCKETS)
        return false;
      if (pHandleEntity->collision_group() ==
...
float proj_aim::travel_time(c_base_player* target, vector weapon_pos, vector pos,
                            vector angle) {
...
double get_speed() {
  ...
  switch (g_cl.m_weapon->item_index()) {
    ...
    case WPN_CompoundBow:
      ...
      charge = (begin_charge == 0.0f)
                   ? 0.0f
                   : TICKS_TO_TIME(g_cl.m_local->m_tick_base()) - begin_charge;
    ...
    case WPN_IronBomber:
      ...
      // Charge = ( BeginCharge == 0.0f ) ? 0.0f : TICKS_TO_TIME( g_cl.m_local->m_tick_base( ) +
      // 1 ) - BeginCharge; weapon_speed = RemapValClamped( Charge, 	0.0f, 	4.f, 	900,
      // 2400 );
      //
      // break;
      weapon_speed = 1200.;
...
vector aim_offset(player_t* player) {
  ...
  vector     offset;
  auto       dir =
      g_cl.m_shoot_pos.look(g_movement.mv.m_position +
  ...
  switch (g_cl.m_weapon->item_index()) {
    ...
    case WPN_CompoundBow:
      offset = target->get_hitbox_pos(0) -
               (target->get_abs_origin()); // -vector( 0, 0, target->get_collideable(
                                           // )->obb_mins( ).m_z ));
...
bool proj_aim::is_pipe() {
  ...
  if (item_id == WPN_IronBomber || item_id == WPN_StickyLauncher ||
      item_id == WPN_LoooseCannon || item_id == WPN_LochNLoad ||
      item_id == WPN_GrenadeLauncher || item_id == WPN_NewGrenadeLauncher ||
...
void proj_aim::select_target() {
  ...
  for (auto i = 1; i <= g_interfaces.m_entity_list->get_highest_entity_index(); i++) {
    ...
    vis_pos.m_z += ((player->get_collideable()->obb_maxs().m_z +
                     player->get_collideable()->obb_mins().m_z) *
                    0.5f);
...
void proj_aim::run() {
  ...
  if (!is_pipe()) {
    ...
    this->m_weapon_speed =
        sqrt(pipe_vel_change[0] * pipe_vel_change[0] + pipe_vel_change[1] * pipe_vel_change[1]);
...
void proj_aim::find_shot(bool& was_shoot, int attack) {
  ...
  for (int steps = 0; steps < maxsteps; steps++) {
    ...
    pos +=
        aim_offset(&target); // +vector( 0, 0, closest->get_collideable( )->obb_mins( ).m_z );;
    ...
    vector new_eye_pos = g_cl.m_shoot_pos;
    {
      ...
      if (is_pipe()) {
        ...
        // view.m_x -= static_cast< float >( atan2( -pipe_vel_change[ 1 ], pipe_vel_change[ 0 ]
        // ) * 180. / pi );
    ...
    if (fabsf(delta) < mindelta &&
        fabsf(delta) < g_interfaces.m_global_vars->m_interval_per_tick * 5) {
...
void GetProjectileFireSetup(vector vecOffset, vector* vecSrc, vector* angForward,
                            float flEndDist = 2000) {
  ...
  CTraceFilterIgnoreTeammates traceFilter(g_cl.m_local, COLLISION_GROUP_NONE,
                                          g_cl.m_local->m_i_team_num());
  g_interfaces.m_engine_trace->trace_ray(ray, MASK_SOLID, &traceFilter, &tr);

  // Offset actual start point
  *vecSrc +=
      vecShootPos + (forward * vecOffset.m_x) + (right * vecOffset.m_y) + (up * vecOffset.m_z);
...
bool proj_aim::setup_projectile(vector& view, vector& pos, vector& new_eyepos) {
  ...
  for (auto i = 0; i < 5; i++) {
    ...
    vector new_eye_pos =
        g_cl.m_shoot_pos + forward * vecOffset.m_x + right * vecOffset.m_y + up * vecOffset.m_z;
    ...
    if (is_pipe()) {
      ...
      // * 180. / pi );
      view.m_x        = std::clamp<float>(view.m_x, -89.f, 89.f);
      // new_view.m_x -= static_cast< float >( atan2( -pipe_vel_change[ 1 ], pipe_vel_change[ 0
      // ] ) * 180. / pi ); new_view.m_x = std::clamp<float>( new_view.m_x, -89.f, 89.f ); view
      // = new_view;
    } else {
      vector delta = get_weapon_delta(g_cl.m_shoot_pos, new_eye_pos, new_view);
      ray_t  ray;
      ray.initialize(new_eye_pos, (delta + new_eye_pos));
      // ray.initialize( new_eyepos, ( delta + new_eyepos ), vector( -8, -8, -8 ), vector( 8, 8,
      // 8 ) );
...
bool proj_aim::get_gravity_aim(vector difference, float* ret, bool lob) {
  ...
  double      root = this->m_weapon_speed * this->m_weapon_speed * this->m_weapon_speed *
                    this->m_weapon_speed -
                this->m_weapon_gravity *
                    (this->m_weapon_gravity * x * x +
  ...
  root = sqrt(root);
  *ret = -rad_to_deg(
      (std::atan2(((this->m_weapon_speed * this->m_weapon_speed) - (lob ? -root : root)),
...
std::vector<vector> proj_aim::path_pred(vector view, float goal_time, vector weapon_pos) {
  ...
  while (time < goal_time) {
    ...
    velocity.m_z -=
        (this->m_weapon_gravity * g_interfaces.m_global_vars->m_interval_per_tick * 0.5f);
...
void proj_aim::draw() {
  ...
  for (auto& i : m_logs) {
    ...
    for (auto i = log.m_path.end() - 2; i != log.m_path.begin(); --i) {
      ...
      //
      //	vector new_dir = path[ i ] + vector( 0, difference + 90.f, 0).angle_vector() *
      // fminf(dif.length(),
      // 15);
      //
...
bool proj_aim::proj_can_hit(c_base_player* target, vector view, float goal_time,
                            vector weapon_pos, bool record) {
  vector eye_pos = g_cl.m_local->m_vec_origin() + g_cl.m_local->m_view_offset();
