...
void prediction::start( ) {
	...
	static auto update_buttons = g_modules.get("client.dll").get_sig("55 8B EC 8B 81 ? ? ? ? 8B D0").as<int( __thiscall* )( c_base_entity*, int )>();
	...
	g_interfaces.m_global_vars->m_cur_time = g_cl.m_local->m_tick_base( ) * g_interfaces.m_global_vars->m_interval_per_tick;
