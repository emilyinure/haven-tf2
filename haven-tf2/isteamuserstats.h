...
struct LeaderboardEntry_t {
  CSteamID m_steamIDUser; // user with the entry - use SteamFriends()->GetFriendPersonaName() &
...
public:
  ...
  // Steam began tracking achievement unlock times (December 2009). Time is seconds since
  // January 1, 1970.
  virtual bool GetAchievementAndUnlockTime(const char* pchName, bool* pbAchieved,
  ...
  // ISteamUtils::GetImageRGBA(), or 0 if none set. A return value of 0 may indicate we are
  // still fetching data, and you can wait for the UserAchievementIconFetched_t callback which
  // will notify you when the bits are ready. If the callback still returns zero, then there is
  // no image set for the specified achievement.
  virtual int GetAchievementIcon(const char* pchName) = 0;

  // Get general attributes for an achievement. Accepts the following keys:
  // - "name" and "desc" for retrieving the localized achievement name and description (returned
  // in UTF8)
  // - "hidden" for retrieving if an achievement is hidden (returns "0" when not hidden, "1"
  // when hidden)
  virtual const char* GetAchievementDisplayAttribute(const char* pchName,
                                                     const char* pchKey) = 0;

  // Achievement progress - triggers an AchievementProgress callback, that is all.
  // Calling this w/ N out of N progress will NOT set the achievement, the game must still do
  // that.
  virtual bool IndicateAchievementProgress(const char* pchName, uint32 nCurProgress,
                                           uint32 nMaxProgress) = 0;

  // Used for iterating achievements. In general games should not need these functions because
  // they should have a list of existing achievements compiled into them
  ...
  // these stats won't be auto-updated; you'll need to call RequestUserStats() again to refresh
  // any data
  ...
  virtual bool GetUserAchievementAndUnlockTime(CSteamID steamIDUser, const char* pchName,
                                               bool* pbAchieved, uint32* punUnlockTime) = 0;
  ...
  virtual ELeaderboardSortMethod
  GetLeaderboardSortMethod(SteamLeaderboard_t hSteamLeaderboard) = 0;

  // returns the display type of the leaderboard
  virtual ELeaderboardDisplayType
  GetLeaderboardDisplayType(SteamLeaderboard_t hSteamLeaderboard) = 0;

  // Asks the Steam back-end for a set of rows in the leaderboard.
  // This call is asynchronous, with the result returned in LeaderboardScoresDownloaded_t
  // LeaderboardScoresDownloaded_t will contain a handle to pull the results from
  // GetDownloadedLeaderboardEntries() (below) You can ask for more entries than exist, and it
  // will return as many as do exist. k_ELeaderboardDataRequestGlobal requests rows in the
  // leaderboard from the full table, with nRangeStart & nRangeEnd in the range [1,
  // TotalEntries] k_ELeaderboardDataRequestGlobalAroundUser requests rows around the current
  // user, nRangeStart being negate
  //   e.g. DownloadLeaderboardEntries( hLeaderboard, k_ELeaderboardDataRequestGlobalAroundUser,
  //   -3, 3 ) will return 7 rows, 3 before the user, 3 after
  ...
  // ELeaderboardDataRequest is k_ELeaderboardDataRequestUsers if a user doesn't have a
  // leaderboard entry, they won't be included in the result a max of 100 users can be
  // downloaded at a time, with only one outstanding call at a time
  STEAM_CALL_RESULT(LeaderboardScoresDownloaded_t)
  virtual SteamAPICall_t
  ...
  // use a for loop from 0 to LeaderboardScoresDownloaded_t::m_cEntryCount to get all the
  // downloaded entries e.g.
  //		void OnLeaderboardScoresDownloaded( LeaderboardScoresDownloaded_t
  ...
  //				int32 details[3];		// we know this is how many
  // we've stored previously 				GetDownloadedLeaderboardEntry(
  // pLeaderboardScoresDownloaded->m_hSteamLeaderboardEntries, index, &leaderboardEntry,
  // details, 3 ); 				assert( leaderboardEntry.m_cDetails == 3 );
  //				...
  //			}
  // once you've accessed all the entries, the data will be free'd, and the
  // SteamLeaderboardEntries_t handle will become invalid
  virtual bool GetDownloadedLeaderboardEntry(SteamLeaderboardEntries_t hSteamLeaderboardEntries,
  ...
  // pScoreDetails points to an array of int32's, cScoreDetailsCount is the number of int32's in
  // the list
  STEAM_CALL_RESULT(LeaderboardScoreUploaded_t)
  virtual SteamAPICall_t
  UploadLeaderboardScore(SteamLeaderboard_t            hSteamLeaderboard,
                         ELeaderboardUploadScoreMethod eLeaderboardUploadScoreMethod,
                         int32 nScore, const int32* pScoreDetails, int cScoreDetailsCount) = 0;

  // Attaches a piece of user generated content the user's entry on a leaderboard.
  // hContent is a handle to a piece of user generated content that was shared using
  // ISteamUserRemoteStorage::FileShare(). This call is asynchronous, with the result returned
  // in LeaderboardUGCSet_t.
  ...
  // Requests that Steam fetch data on the percentage of players who have received each
  // achievement for the game globally. This call is asynchronous, with the result returned in
  // GlobalAchievementPercentagesReady_t.
  STEAM_CALL_RESULT(GlobalAchievementPercentagesReady_t)
  virtual SteamAPICall_t RequestGlobalAchievementPercentages() = 0;

  // Get the info on the most achieved achievement for the game, returns an iterator index you
  // can use to fetch the next most achieved afterwards.  Will return -1 if there is no data on
  // achievement percentages (ie, you haven't called RequestGlobalAchievementPercentages and
  // waited on the callback).
  virtual int GetMostAchievedAchievementInfo(char* pchName, uint32 unNameBufLen,
                                             float* pflPercent, bool* pbAchieved) = 0;

  // Get the info on the next most achieved achievement for the game. Call this after
  // GetMostAchievedAchievementInfo or another GetNextMostAchievedAchievementInfo call passing
  // the iterator from the previous call. Returns -1 after the last achievement has been
  // iterated.
  virtual int GetNextMostAchievedAchievementInfo(int iIteratorPrevious, char* pchName,
  ...
  // Gets history for an aggregated stat. pData will be filled with daily values, starting with
  // today. So when called, pData[0] will be today, pData[1] will be yesterday, and pData[2]
  // will be two days ago, etc. cubData is the size in bytes of the pubData buffer. Returns the
  ...
  virtual int32 GetGlobalStatHistory(const char*                       pchStatName,
                                     STEAM_ARRAY_COUNT(cubData) int64* pData,
                                     uint32                            cubData) = 0;

  STEAM_FLAT_NAME(GetGlobalStatHistoryDouble)
  virtual int32 GetGlobalStatHistory(const char*                        pchStatName,
                                     STEAM_ARRAY_COUNT(cubData) double* pData,
                                     uint32                             cubData) = 0;

  // For achievements that have related Progress stats, use this to query what the bounds of
  // that progress are. You may want this info to selectively call IndicateAchievementProgress
  // when appropriate milestones of progress have been made, to show a progress notification to
  // the user.
  STEAM_FLAT_NAME(GetAchievementProgressLimitsInt32)
  virtual bool GetAchievementProgressLimits(const char* pchName, int32* pnMinProgress,
                                            int32* pnMaxProgress) = 0;

  STEAM_FLAT_NAME(GetAchievementProgressLimitsFloat)
  virtual bool GetAchievementProgressLimits(const char* pchName, float* pfMinProgress,
                                            float* pfMaxProgress) = 0;
};
...
STEAM_DEFINE_USER_INTERFACE_ACCESSOR(ISteamUserStats*, SteamUserStats,
...
struct LeaderboardFindResult_t {
  ...
  SteamLeaderboard_t
      m_hSteamLeaderboard; // handle to the leaderboard serarched for, 0 if no leaderboard found
  uint8 m_bLeaderboardFound; // 0 if no leaderboard found
};

//-----------------------------------------------------------------------------
// Purpose: call result indicating scores for a leaderboard have been downloaded and are ready
// to be retrieved, returned as a result of DownloadLeaderboardEntries()
//			use CCallResult<> to map this async result to a member function
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: call result indicating scores has been uploaded, returned as a result of
...
struct LeaderboardScoreUploaded_t {
  ...
  uint8 m_bScoreChanged;  // true if the score in the leaderboard change, false if the existing
                          // score was better
  int   m_nGlobalRankNew; // the new global rank of the user in this leaderboard
  int   m_nGlobalRankPrevious; // the previous global rank of the user in this leaderboard; 0 if
                               // the user had no existing entry in the leaderboard
};
...
struct UserAchievementIconFetched_t {
  ...
  int m_nIconHandle; // Handle to the image, which can be used in SteamUtils()->GetImageRGBA(),
...
//-----------------------------------------------------------------------------
// Purpose: call result indicating UGC has been uploaded, returned as a result of
// SetLeaderboardUGC()
//-----------------------------------------------------------------------------
...
struct PS3TrophiesInstalled_t {
  ...
  uint64  m_nGameID;            // Game these stats are for
  EResult m_eResult;            // The result of the operation
  uint64 m_ulRequiredDiskSpace; // If m_eResult is k_EResultDiskFull, will contain the amount of
                                // space needed to install trophies
};
