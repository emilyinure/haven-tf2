...
//    Note: The older ISteamController interface has been deprecated in favor of ISteamInput -
//    this interface
//			was updated in this SDK but will be removed from future SDK's. The Steam
// Client will retain 			compatibility with the older interfaces so your any
...
// When sending an option to a specific controller handle, you can send to all controllers via
// this command
...
public:
  ...
  // handlesOut should point to a STEAM_CONTROLLER_MAX_COUNT sized array of ControllerHandle_t
  // handles Returns the number of handles written to handlesOut
  virtual int GetConnectedControllers(
      STEAM_OUT_ARRAY_COUNT(STEAM_CONTROLLER_MAX_COUNT, Receives list of connected controllers)
  ...
  virtual ControllerActionSetHandle_t
  GetCurrentActionSet(ControllerHandle_t controllerHandle) = 0;
  ...
  // handlesOut should point to a STEAM_CONTROLLER_MAX_ACTIVE_LAYERS sized array of
  // ControllerActionSetHandle_t handles. Returns the number of handles written to handlesOut
  ...
  virtual ControllerDigitalActionData_t
  GetDigitalActionData(ControllerHandle_t              controllerHandle,
                       ControllerDigitalActionHandle_t digitalActionHandle) = 0;
  ...
  // action. originsOut should point to a STEAM_CONTROLLER_MAX_ORIGINS sized array of
  // EControllerActionOrigin handles. The EControllerActionOrigin enum will get extended as
  // support for new controller controllers gets added to the Steam client and will exceed the
  // values from this header, please check bounds if you are using a look up table.
  ...
  virtual ControllerAnalogActionData_t
  ...
  // Returns a localized string (from Steam's language setting) for the specified origin - this
  // call is serialized
  ...
  virtual void TriggerRepeatedHapticPulse(ControllerHandle_t  controllerHandle,
                                          ESteamControllerPad eTargetPad,
  ...
  virtual void SetLEDColor(ControllerHandle_t controllerHandle, uint8 nColorR, uint8 nColorG,
  ...
  // Returns the input type for a particular handle - unlike EControllerActionOrigin which
  // update with Steam and may return unrecognized values ESteamInputType will remain static and
  // only return valid values from your SDK version
  ...
  virtual EControllerActionOrigin
  GetActionOriginFromXboxOrigin(ControllerHandle_t controllerHandle, EXboxOrigin eOrigin) = 0;

  // Convert an origin to another controller type - for inputs not present on the other
  // controller type this will return k_EControllerActionOrigin_None
  virtual EControllerActionOrigin
  ...
  virtual bool GetControllerBindingRevision(ControllerHandle_t controllerHandle, int* pMajor,
                                            int* pMinor) = 0;
};
...
STEAM_DEFINE_USER_INTERFACE_ACCESSOR(ISteamController*, SteamController,
