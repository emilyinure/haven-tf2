...
//
// Purpose: Steam Input is a flexible input API that supports over three hundred devices
// including all
...
// When sending an option to a specific controller handle, you can send to all devices via this
// command
...
// Versions of Steam that add new controller types in the future will extend this enum so if
// you're using a lookup table please check the bounds of any origins returned by Steam.
enum EInputActionOrigin {
  ...
  k_EInputActionOrigin_Switch_RightGyro_Move,  // Right JoyCon Gyro generally should correspond
                                               // to Pro's single gyro
  k_EInputActionOrigin_Switch_RightGyro_Pitch, // Right JoyCon Gyro generally should correspond
                                               // to Pro's single gyro
  k_EInputActionOrigin_Switch_RightGyro_Yaw, // Right JoyCon Gyro generally should correspond to
                                             // Pro's single gyro
  k_EInputActionOrigin_Switch_RightGyro_Roll, // Right JoyCon Gyro generally should correspond
                                              // to Pro's single gyro
  ...
  k_EInputActionOrigin_Switch_JoyConButton_N, // With a Horizontal JoyCon this will be Y or what
                                              // would be Dpad Right when vertical
  ...
  k_EInputActionOrigin_Count, // If Steam has added support for new controllers origins will go
                              // here.
...
enum EControllerHapticLocation {
  ...
  k_EControllerHapticLocation_Both =
      (1 << k_ESteamControllerPad_Left | 1 << k_ESteamControllerPad_Right),
};
...
enum ESteamInputLEDFlag {
  ...
  // Restore the LED color to the user's preference setting as set in the controller
  // personalization menu. This also happens automatically on exit of your game.
...
enum ESteamInputGlyphStyle {
  ...
  // Modifiers
  // Default ABXY/PS equivalent glyphs have a solid fill w/ color matching the physical buttons
  // on the device
  ESteamInputGlyphStyle_NeutralColorABXY = 0x10, // ABXY Buttons will match the base style color
                                                 // instead of their normal associated color
...
struct InputMotionDataV2_t {
  ...
  // Drift Corrected Quaternion is calculated after steam input controller calibration values
  // have been applied. Rawest _useful_ version of a quaternion. Most camera implementations
  // should use this by comparing last rotation against current rotation, and applying the
  // difference to the in game camera (plus your own sensitivity tweaks) It is worth viewing
  ...
  // This can "ouija" your aim, so it's not so  appropriate for camera controls (sensor fusion
  // was originally made for racing game steering ) Same result as from old
  // InputMotionData_t::rotQuatX/Y/Z/W
  ...
  // Reduces perception of "ouija" effect by only applying correction when the controller is
  // below "low noise" thresholds, while the controller rotates fast - never when the user is
  // attempting precision aim.
...
public:
  ...
  virtual bool
  SetInputActionManifestFilePath(const char* pchInputActionManifestAbsolutePath) = 0;
  ...
  // Enumerate currently connected Steam Input enabled devices - developers can opt in
  // controller by type (ex: Xbox/Playstation/etc) via the Steam Input settings in the
  // Steamworks site or users can opt-in in their controller settings in Steam. handlesOut
  // should point to a STEAM_INPUT_MAX_COUNT sized array of InputHandle_t handles Returns the
  // number of handles written to handlesOut
  virtual int GetConnectedControllers(
      STEAM_OUT_ARRAY_COUNT(STEAM_INPUT_MAX_COUNT, Receives list of connected controllers)
  ...
  // a handler. Note: this is called within either SteamInput()->RunFrame or by
  ...
  // Controllers using Gamepad emulation (XInput, DirectInput, etc) will be seated in the order
  // that input is sent by the device. This callback will fire on first input for each device
  // and when the a user has manually changed the order via the Steam overlay. This also has the
  // device type info so that you can change out glyph sets without making additional API calls
  ...
  // Lookup the handle for an Action Set. Best to do this once on startup, and store the handles
  // for all future API calls.
  virtual InputActionSetHandle_t GetActionSetHandle(const char* pszActionSetName) = 0;

  // Reconfigure the controller to use the specified action set (ie 'Menu', 'Walk' or 'Drive')
  // This is cheap, and can be safely called repeatedly. It's often easier to repeatedly call it
  // in your state loops, instead of trying to place it in all of your state transitions.
  virtual void                   ActivateActionSet(InputHandle_t          inputHandle,
                                                   InputActionSetHandle_t actionSetHandle) = 0;
  virtual InputActionSetHandle_t GetCurrentActionSet(InputHandle_t inputHandle)            = 0;

  // ACTION SET LAYERS
  virtual void ActivateActionSetLayer(InputHandle_t          inputHandle,
  ...
  // handlesOut should point to a STEAM_INPUT_MAX_ACTIVE_LAYERS sized array of
  // InputActionSetHandle_t handles Returns the number of handles written to handlesOut
  ...
  // Lookup the handle for a digital action. Best to do this once on startup, and store the
  ...
  virtual InputDigitalActionData_t
  GetDigitalActionData(InputHandle_t              inputHandle,
                       InputDigitalActionHandle_t digitalActionHandle) = 0;

  // Get the origin(s) for a digital action within an action set. Returns the number of origins
  // supplied in originsOut. Use this to display the appropriate on-screen prompt for the
  // action. originsOut should point to a STEAM_INPUT_MAX_ORIGINS sized array of
  // EInputActionOrigin handles. The EInputActionOrigin enum will get extended as support for
  // new controller controllers gets added to the Steam client and will exceed the values from
  // this header, please check bounds if you are using a look up table.
  virtual int GetDigitalActionOrigins(InputHandle_t              inputHandle,
  ...
  // Returns a localized string (from Steam's language setting) for the user-facing action name
  // corresponding to the specified handle
  virtual const char*
  GetStringForDigitalActionName(InputDigitalActionHandle_t eActionHandle) = 0;

  // Lookup the handle for an analog action. Best to do this once on startup, and store the
  // handles for all future API calls.
  virtual InputAnalogActionHandle_t GetAnalogActionHandle(const char* pszActionName) = 0;

  // Returns the current state of these supplied analog game action
  virtual InputAnalogActionData_t
  GetAnalogActionData(InputHandle_t             inputHandle,
                      InputAnalogActionHandle_t analogActionHandle) = 0;

  // Get the origin(s) for an analog action within an action set. Returns the number of origins
  // supplied in originsOut. Use this to display the appropriate on-screen prompt for the
  // action. originsOut should point to a STEAM_INPUT_MAX_ORIGINS sized array of
  // EInputActionOrigin handles. The EInputActionOrigin enum will get extended as support for
  // new controller controllers gets added to the Steam client and will exceed the values from
  // this header, please check bounds if you are using a look up table.
  virtual int GetAnalogActionOrigins(InputHandle_t             inputHandle,
                                     InputActionSetHandle_t    actionSetHandle,
  ...
  virtual const char* GetGlyphPNGForActionOrigin(EInputActionOrigin   eOrigin,
                                                 ESteamInputGlyphSize eSize,
                                                 uint32               unFlags) = 0;

  // Get a local path to a SVG file for the provided origin's glyph.
  virtual const char* GetGlyphSVGForActionOrigin(EInputActionOrigin eOrigin,
                                                 uint32             unFlags) = 0;
  ...
  virtual void StopAnalogActionMomentum(InputHandle_t             inputHandle,
                                        InputAnalogActionHandle_t eAction) = 0;
  ...
  // Trigger a vibration event on supported controllers - Steam will translate these commands
  ...
  // Trigger a vibration event on supported controllers including Xbox trigger impulse rumble -
  // Steam will translate these commands into haptic pulses for Steam Controllers
  ...
  virtual void TriggerSimpleHapticEvent(InputHandle_t             inputHandle,
                                        EControllerHapticLocation eHapticLocation,
                                        uint8 nIntensity, char nGainDB, uint8 nOtherIntensity,
                                        char nOtherGainDB) = 0;

  // Set the controller LED color on supported controllers. nFlags is a bitmask of values from
  // ESteamInputLEDFlag - 0 will default to setting a color. Steam will handle the behavior on
  // exit of your program so you don't need to try restore the default as you are shutting down
  virtual void SetLEDColor(InputHandle_t inputHandle, uint8 nColorR, uint8 nColorG,
                           uint8 nColorB, unsigned int nFlags) = 0;

  // Trigger a haptic pulse on a Steam Controller - if you are approximating rumble you may want
  // to use TriggerVibration instead. Good uses for Haptic pulses include chimes, noises, or
  // directional gameplay feedback (taking damage, footstep locations, etc).
  virtual void Legacy_TriggerHapticPulse(InputHandle_t       inputHandle,
                                         ESteamControllerPad eTargetPad,
                                         unsigned short      usDurationMicroSec) = 0;

  // Trigger a haptic pulse with a duty cycle of usDurationMicroSec / usOffMicroSec, unRepeat
  // times. If you are approximating rumble you may want to use TriggerVibration instead. nFlags
  // is currently unused and reserved for future use.
  virtual void Legacy_TriggerRepeatedHapticPulse(InputHandle_t       inputHandle,
                                                 ESteamControllerPad eTargetPad,
  ...
  // Invokes the Steam overlay and brings up the binding screen if the user is using Big Picture
  // Mode If the user is not in Big Picture Mode it will open up the binding in a new window
  virtual bool ShowBindingPanel(InputHandle_t inputHandle) = 0;

  // Returns the input type for a particular handle - unlike EInputActionOrigin which update
  // with Steam and may return unrecognized values ESteamInputType will remain static and only
  // return valid values from your SDK version
  virtual ESteamInputType GetInputTypeForHandle(InputHandle_t inputHandle) = 0;

  // Returns the associated controller handle for the specified emulated gamepad - can be used
  // with the above 2 functions to identify controllers presented to your game over Xinput.
  // Returns 0 if the Xinput index isn't associated with Steam Input
  virtual InputHandle_t GetControllerForGamepadIndex(int nIndex) = 0;

  // Returns the associated gamepad index for the specified controller, if emulating a gamepad
  // or -1 if not associated with an Xinput index
  virtual int GetGamepadIndexForController(InputHandle_t ulinputHandle) = 0;

  // Returns a localized string (from Steam's language setting) for the specified Xbox
  // controller origin.
  ...
  // Get the equivalent ActionOrigin for a given Xbox controller origin this can be chained with
  // GetGlyphForActionOrigin to provide future proof glyphs for non-Steam Input API action
  // games. Note - this only translates the buttons directly and doesn't take into account any
  // remapping a user has made in their configuration
  virtual EInputActionOrigin GetActionOriginFromXboxOrigin(InputHandle_t inputHandle,
                                                           EXboxOrigin   eOrigin) = 0;

  // Convert an origin to another controller type - for inputs not present on the other
  // controller type this will return k_EInputActionOrigin_None When a new input type is added
  // you will be able to pass in k_ESteamInputType_Unknown and the closest origin that your
  // version of the SDK recognized will be returned ex: if a Playstation 5 controller was
  // released this function would return Playstation 4 origins.
  virtual EInputActionOrigin TranslateActionOrigin(ESteamInputType    eDestinationInputType,
                                                   EInputActionOrigin eSourceOrigin) = 0;

  // Get the binding revision for a given device. Returns false if the handle was not valid or
  // if a mapping is not yet loaded for the device
  virtual bool GetDeviceBindingRevision(InputHandle_t inputHandle, int* pMajor,
                                        int* pMinor) = 0;

  // Get the Steam Remote Play session ID associated with a device, or 0 if there is no session
  // associated with it See isteamremoteplay.h for more information on Steam Remote Play
  // sessions
  ...
  // Returns ESteamInputConfigurationEnableType values. Note: user can override the settings
  // from the Steamworks Partner site so the returned values may not exactly match your default
  // configuration
  virtual uint16 GetSessionInputConfigurationSettings() = 0;

  // Set the trigger effect for a DualSense controller
  virtual void SetDualSenseTriggerEffect(InputHandle_t                   inputHandle,
                                         const ScePadTriggerEffectParam* pParam) = 0;
};
...
struct SteamInputGamepadSlotChange_t {
  ...
  int m_nOldGamepadSlot; // Previous GamepadSlot - can be -1 controller doesn't uses gamepad
                         // bindings
  int m_nNewGamepadSlot; // New Gamepad Slot - can be -1 controller doesn't uses gamepad
                         // bindings
};
