...
enum EFriendRelationship {
  ...
  k_EFriendRelationshipSuggested_DEPRECATED =
      7, // was used by the original implementation of the facebook linking feature, but now
         // unused.
...
//-----------------------------------------------------------------------------
// Purpose: flags for enumerating friends list, or quickly checking a the relationship between
// users
//-----------------------------------------------------------------------------
enum EFriendFlags {
...
// The UTF-8 version has to be very generous to accomodate characters that get large when
// encoded in UTF-8.
enum {
...
enum EActivateGameOverlayToWebPageMode {
  ...
  k_EActivateGameOverlayToWebPageMode_Modal =
      1 // Browser will be opened in a special overlay configuration which hides all other
        // windows that the user has open in the overlay. When the user closes the overlay, the
        // browser window will also close. When the user closes the browser window, the overlay
...
//			that can be a friend, in a group, on a game server or in a lobby with
// the local user
//-----------------------------------------------------------------------------
class ISteamFriends {
public:
  ...
  // like all the other interface functions that return a char *, it's important that this
  // pointer is not saved off; it will eventually be free'd or re-allocated
  virtual const char* GetPersonaName() = 0;

  // Sets the player name, stores it on the server and publishes the changes to all friends who
  // are online. Changes take place locally immediately, and a PersonaStateChange_t is posted,
  // presuming success.
  //
  // The final results are available through the return value SteamAPICall_t, using
  // SetPersonaNameResponse_t.
  //
  // If the name change fails to happen on the server, then an additional global
  // PersonaStateChange_t will be posted to change the name back, in addition to the
  // SetPersonaNameResponse_t callback.
  ...
  // friend iteration
  // takes a set of k_EFriendFlags, and returns the number of users the client knows about who
  // meet that criteria then GetFriendByIndex() can then be used to return the id's of each of
  // those users
  ...
  // the returned CSteamID can then be used by all the functions below to access details about
  // the user
  ...
  // this will only be known by the local user if steamIDFriend is in their friends list; on the
  // same game server; in a chat room or lobby; or in a small group with the local user
  virtual EPersonaState GetFriendPersonaState(CSteamID steamIDFriend) = 0;

  // returns the name another user - guaranteed to not be NULL.
  // same rules as GetFriendPersonaState() apply as to whether or not the user knowns the name
  // of the other user note that on first joining a lobby, chat room or game server the local
  // user will not known the name of the other users automatically; that information will arrive
  // asyncronously
  //
  virtual const char* GetFriendPersonaName(CSteamID steamIDFriend) = 0;

  // returns true if the friend is actually in a game, and fills in pFriendGameInfo with an
  // extra details
  virtual bool        GetFriendGamePlayed(CSteamID                             steamIDFriend,
                                          STEAM_OUT_STRUCT() FriendGameInfo_t* pFriendGameInfo) = 0;
  // accesses old friends names - returns an empty string when their are no more items in the
  // history
  virtual const char* GetFriendPersonaNameHistory(CSteamID steamIDFriend, int iPersonaName) = 0;
  // friends steam level
  virtual int         GetFriendSteamLevel(CSteamID steamIDFriend)                           = 0;

  // Returns nickname the current user has set for the specified player. Returns NULL if the no
  // nickname has been set for that player. DEPRECATED: GetPersonaName follows the Steam
  // nickname preferences, so apps shouldn't need to care about nicknames explicitly.
  ...
  // returns the name for the given friends group (NULL in the case of invalid friends group
  // IDs)
  ...
  // gets up to nMembersCount members of the given friends group, if fewer exist than requested
  // those positions' SteamIDs will be invalid
  virtual void
  GetFriendsGroupMembersList(FriendsGroupID_t                               friendsGroupID,
  ...
  virtual bool        GetClanActivityCounts(CSteamID steamIDClan, int* pnOnline, int* pnInGame,
                                            int* pnChatting) = 0;

  // for clans a user is a member of, they will have reasonably up-to-date information, but for
  // others you'll have to download the info to have the latest
  STEAM_CALL_RESULT(DownloadClanActivityCountsResult_t)
  virtual SteamAPICall_t DownloadClanActivityCounts(STEAM_ARRAY_COUNT(cClansToRequest)
  ...
  // lobby steamIDSource can be the steamID of a group, game server, lobby or chat room
  ...
  // User is in a game pressing the talk button (will suppress the microphone for all voice
  // comms from the Steam friends UI)
  virtual void SetInGameVoiceSpeaking(CSteamID steamIDUser, bool bSpeaking) = 0;

  // activates the game overlay, with an optional dialog to open
  // valid options include "Friends", "Community", "Players", "Settings", "OfficialGameGroup",
  // "Stats", "Achievements", "chatroomgroup/nnnn"
  virtual void ActivateGameOverlay(const char* pchDialog) = 0;

  // activates game overlay to a specific place
  // valid options are
  //		"steamid" - opens the overlay web browser to the specified user or groups
  // profile 		"chat" - opens a chat window to the specified user, or joins the group
  // chat 		"jointrade" - opens a window to a Steam Trading session that was started with the
  // ISteamEconomy/StartTrade Web API 		"stats" - opens the overlay web browser to the
  // specified user's stats 		"achievements" - opens the overlay web browser to the
  // specified user's achievements 		"friendadd" - opens the overlay in minimal mode
  // prompting the user to add the target user as a friend 		"friendremove" - opens
  // the overlay in minimal mode prompting the user to remove the target friend
  // "friendrequestaccept" - opens the overlay in minimal mode prompting the user to accept an
  // incoming friend invite 		"friendrequestignore" - opens the overlay in minimal
  // mode prompting the user to ignore an incoming friend invite
  virtual void ActivateGameOverlayToUser(const char* pchDialog, CSteamID steamID) = 0;

  // activates game overlay web browser directly to the specified URL
  // full address with protocol type is required, e.g. http://www.steamgames.com/
  virtual void
  ActivateGameOverlayToWebPage(const char*                       pchURL,
  ...
  // Mark a target user as 'played with'. This is a client-side only feature that requires that
  // the calling user is in game
  virtual void SetPlayedWith(CSteamID steamIDUserPlayedWith) = 0;

  // activates game overlay to open the invite dialog. Invitations will be sent for the provided
  // lobby.
  virtual void ActivateGameOverlayInviteDialog(CSteamID steamIDLobby) = 0;

  // gets the small (32x32) avatar of the current user, which is a handle to be used in
  // IClientUtils::GetImageRGBA(), or 0 if none set
  virtual int GetSmallFriendAvatar(CSteamID steamIDFriend) = 0;

  // gets the medium (64x64) avatar of the current user, which is a handle to be used in
  // IClientUtils::GetImageRGBA(), or 0 if none set
  virtual int GetMediumFriendAvatar(CSteamID steamIDFriend) = 0;

  // gets the large (184x184) avatar of the current user, which is a handle to be used in
  // IClientUtils::GetImageRGBA(), or 0 if none set returns -1 if this image has yet to be
  // loaded, in this case wait for a AvatarImageLoaded_t callback and then call this again
  virtual int GetLargeFriendAvatar(CSteamID steamIDFriend) = 0;

  // requests information about a user - persona name & avatar
  // if bRequireNameOnly is set, then the avatar of a user isn't downloaded
  // - it's a lot slower to download avatars and churns the local cache, so if you don't need
  // avatars, don't request them if returns true, it means that data is being requested, and a
  // PersonaStateChanged_t callback will be posted when it's retrieved if returns false, it
  // means that we already have all the details about that user, and functions can be called
  // immediately
  ...
  // and no avatar image is available, call RequestUserInformation( steamID, false ) to download
  // the avatar
  STEAM_CALL_RESULT(ClanOfficerListResponse_t)
  virtual SteamAPICall_t RequestClanOfficerList(CSteamID steamIDClan) = 0;

  // iteration of clan officers - can only be done when a RequestClanOfficerList() call has
  // completed
  ...
  // Note the following limits: k_cchMaxRichPresenceKeys, k_cchMaxRichPresenceKeyLength,
  // k_cchMaxRichPresenceValueLength There are five magic keys:
  //		"status"  - a UTF-8 string that will show up in the 'view game info' dialog in
  // the Steam friends list 		"connect" - a UTF-8 string that contains the
  // command-line for how a friend can connect to a game 		"steam_display"
  // - Names a rich presence localization token that will be displayed in the viewing user's
  // selected language 									  in the Steam client UI. For more info:
  // https://partner.steamgames.com/doc/api/ISteamFriends#richpresencelocalization
  //		"steam_player_group"		- When set, indicates to the Steam client that
  // the player is a member of a particular group. Players in the same group
  ...
  // steam_player_group. The Steam client may use this number to
  // display additional information about a group when all of the members are not part of a
  // user's friends list.
  ...
  // Rich invite support.
  // If the target accepts the invite, a GameRichPresenceJoinRequested_t callback is posted
  // containing the connect string. (Or you can configure your game so that it is passed on the
  // command line instead.  This is a deprecated path; ask us if you really need this.)
  ...
  // the behavior is somewhat sophisticated, because the user may or may not be already in the
  // group chat from outside the game or in the overlay use ActivateGameOverlayToUser( "chat",
  // steamIDClan ) to open the in-game overlay version of the chat
  ...
  virtual int  GetClanChatMessage(CSteamID steamIDClanChat, int iMessage, void* prgchText,
                                  int cchTextMax, EChatEntryType* peChatEntryType,
  ...
  virtual int  GetFriendMessage(CSteamID steamIDFriend, int iMessageID, void* pvData,
  ...
  // activates game overlay to open the remote play together invite dialog. Invitations will be
  // sent for remote play together
  virtual void ActivateGameOverlayRemotePlayTogetherInviteDialog(CSteamID steamIDLobby) = 0;

  // Call this before calling ActivateGameOverlayToWebPage() to have the Steam Overlay Browser
  // block navigations to your specified protocol (scheme) uris and instead dispatch a
  // OverlayBrowserProtocolNavigation_t callback to your game. ActivateGameOverlayToWebPage()
  // must have been called with k_EActivateGameOverlayToWebPageMode_Modal
  virtual bool RegisterProtocolInOverlayBrowser(const char* pchProtocol) = 0;

  // Activates the game overlay to open an invite dialog that will send the provided Rich
  // Presence connect string to selected friends
  ...
  // You can register for EquippedProfileItemsChanged_t to know when a friend has changed their
  ...
  virtual bool           BHasEquippedProfileItem(CSteamID                  steamID,
                                                 ECommunityProfileItemType itemType)      = 0;
  virtual const char*    GetProfileItemPropertyString(CSteamID                      steamID,
                                                      ECommunityProfileItemType     itemType,
                                                      ECommunityProfileItemProperty prop) = 0;
  virtual uint32         GetProfileItemPropertyUint(CSteamID                      steamID,
                                                    ECommunityProfileItemType     itemType,
                                                    ECommunityProfileItemProperty prop)   = 0;
};
...
STEAM_DEFINE_USER_INTERFACE_ACCESSOR(ISteamFriends*, SteamFriends,
...
// these flags describe what the client has learned has changed recently, so on startup you'll
// see a name, avatar & relationship change for every friend
enum EPersonaChange {
...
//			game client should attempt to connect to specified server when this is
// received
//-----------------------------------------------------------------------------
...
//			game client should attempt to connect to specified lobby when this is
// received
//-----------------------------------------------------------------------------
struct GameLobbyJoinRequested_t {
  ...
  // the account type will be console user so you can tell at least that this was from a PSN
  // friend rather than a Steam friend.
...
//			rich presence will have been set with the "connect" key which is set
// here
//-----------------------------------------------------------------------------
struct GameRichPresenceJoinRequested_t {
  enum { k_iCallback = k_iSteamFriendsCallbacks + 37 };
  CSteamID m_steamIDFriend; // the friend they did the join via (will be invalid if not directly
                            // via a friend)
...
struct SetPersonaNameResponse_t {
  ...
  bool    m_bLocalSuccess; // true if name change was retained locally.  (We might not have been
                           // able to communicate with Steam)
...
//-----------------------------------------------------------------------------
// Purpose: Dispatched when an overlay browser instance is navigated to a protocol/scheme
// registered by RegisterProtocolInOverlayBrowser()
//-----------------------------------------------------------------------------
