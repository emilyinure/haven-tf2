...
void c_interfaces::gather() {
  const auto client = g_modules.get("client.dll");
  {
    ...
    // this->m_game_rules = **client.get_sig("8B 0D ? ? ? ? 83 C4 10 C7
    // 45").as<game_rules***>(0x2);
    this->m_client_mode =
        *reinterpret_cast<void**>(client.get_sig("48 8B 0D ? ? ? ? 45 33 C9 FF 93").rel32(0x3));
    this->m_global_vars = *reinterpret_cast<c_global_vars_base**>(
        client.get_sig("48 8B 05 ?? ?? ?? ?? 48 8D 56 18").rel32(0x3));
    this->steam_friends = *reinterpret_cast<ISteamFriends**>(
        client
            .get_sig("48 8B 0D ?? ?? ?? ?? 48 85 C9 74 1C 48 8B 49 10 48 85 C9 74 13 48 8B 01 "
                     "48 8D 15 ?? ?? ?? "
  ...
  const auto engine = g_modules.get("engine.dll");
  {
    ...
    this->m_engine_trace =
        engine.get_interface("EngineTraceClient003", true).as<i_engine_trace>();
    this->m_model_info = engine.get_interface("VModelInfoClient006", true).as<i_model_info>();
    this->m_render_view =
        engine.get_interface("VEngineRenderView014", true).as<c_render_view>();
    this->m_client_state = *reinterpret_cast<c_client_state**>(
        engine.get_sig("48 8D 0D ? ? ? ? E8 ? ? ? ? F3 0F 5E 05").rel32(0x3));
  }
  ...
  const auto steam = g_modules.get("steamclient64.dll");
  {
    ...
    steam_friends =
        steam_client->GetISteamFriends(hsNewUser, hsNewPipe, STEAMFRIENDS_INTERFACE_VERSION);
    ...
    steam_user_stats = steam_client->GetISteamUserStats(hsNewUser, hsNewPipe,
  ...
  m_mem_alloc = *reinterpret_cast<mem_alloc_t**>(
      GetProcAddress(GetModuleHandleA("tier0.dll"), "g_pMemAlloc"));
}

void c_interfaces::init() { this->gather(); }
