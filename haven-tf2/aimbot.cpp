...
public:
	virtual bool should_hit_entity( c_base_entity* pEntityHandle, int contentsMask ) {
		return ( pEntityHandle->entindex( ) == 0 || pEntityHandle->entindex( ) > g_interfaces.m_engine->get_max_clients( ) ) || pEntityHandle == pHit;
	}
...
void c_aimbot::other( c_base_player* local, usercmd_t* cmd ) {
	...
	if ( !weapon || weapon->GetSlot( ) != EWeaponSlots::SLOT_PRIMARY || (TICKS_TO_TIME( local->m_tick_base( ) + 1 ) < weapon->m_next_primary_attack( ) && local->m_class( ) != TF2_Heavy) )
		return;
...
void c_aimbot::run( c_base_player* local, usercmd_t* cmd ) {
	...
	if ( item_idx == WPN_Huntsman || item_idx == WPN_CompoundBow || TICKS_TO_TIME( local->m_tick_base( ) + 1 ) < weapon->m_next_primary_attack( ) )
		return;
