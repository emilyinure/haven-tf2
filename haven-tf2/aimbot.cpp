...
void c_aimbot::other(c_base_player* local, usercmd_t* cmd) {
  ...
  if (local->m_class() != TF2_Sniper && local->m_class() != TF2_Scout &&
      local->m_class() != TF2_Heavy && !soldier)
    return;
  ...
  if (soldier) {
    ...
    return;
  if ((TICKS_TO_TIME(local->m_tick_base() + 1) < weapon->m_next_primary_attack()) &&
      local->m_class() != TF2_Heavy)
    return;
...
void c_aimbot::run(c_base_player* local, usercmd_t* cmd) {
  ...
  if ((TICKS_TO_TIME(local->m_tick_base() + 1) < local->m_next_attack() &&
       local->m_class() != TF2_Heavy))
    return;
  auto* weapon = local->get_active_weapon();
  if (!weapon)
  ...
  for (auto i = 1; i <= g_interfaces.m_entity_list->get_highest_entity_index(); i++) {
    ...
    player->maxs() *= 10.f;
    record->cache();
    // g_interfaces.m_debug_overlay->add_box_overlay(head, vector(-8, -8, -8), vector(8, 8, 8),
    // vector(0, 0, 0), 255,
    //                                               0, 0, 255, 0.1);
    ray_t ray;
    ray.initialize(eye_pos, head);
