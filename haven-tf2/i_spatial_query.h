...
struct ray_t
{
    ...
    void initialize(vector const& start, vector const& end, vector const& mins, vector const& maxs)
    {
        m_Delta = end - start;
    ...
    ray_t(vector vecStart, vector vecEnd)
    {
...
public:
    ...
    virtual bool enumerate_leaves_in_box(vector const& mins, vector const& maxs, i_spatial_leaf_enumerator* p_enum,
                                         int context) = 0;
    virtual bool enumerate_leaves_in_sphere(vector const& center, float radius, i_spatial_leaf_enumerator* p_enum,
                                            int context) = 0;
