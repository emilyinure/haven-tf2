...
public:
  ...
  /// (SteamNetworkingIPAddr::Clear()). This means that you will not bind to any particular
  /// local interface (i.e. the same as INADDR_ANY in plain socket code).  Furthermore, if
  /// possible the socket will be bound in "dual stack" mode, which means that it can accept
  /// both IPv4 and IPv6 client connections. If you really do wish to bind a particular
  /// interface, then set the local address to the appropriate IPv4 or IPv6 IP.
  ///
  ...
  virtual HSteamListenSocket
  CreateListenSocketIP(const SteamNetworkingIPAddr& localAddress, int nOptions,
  ...
  /// SteamNetworkingConfigValue_t for more about why this is preferable to
  /// setting the options "immediately" after creation.
  virtual HSteamNetConnection
  ConnectByIPAddress(const SteamNetworkingIPAddr& address, int nOptions,
                     const SteamNetworkingConfigValue_t* pOptions) = 0;
  ...
  /// then you can listen using this function instead of
  /// CreateHostedDedicatedServerListenSocket, to allow clients to connect without a ticket. Any
  /// user that owns the app and is signed into Steam will be able to attempt to connect to your
  /// server.  Also, a connection attempt may require the client to be connected to Steam, which
  /// is one more moving part that may fail.  When tickets are used, then once a ticket is
  /// obtained, a client can connect to your server even if they got disconnected from Steam or
  /// Steam is offline.
  ///
  /// If you need to set any initial config options, pass them here.  See
  /// SteamNetworkingConfigValue_t for more about why this is preferable to
  /// setting the options "immediately" after creation.
  virtual HSteamListenSocket
  CreateListenSocketP2P(int nLocalVirtualPort, int nOptions,
                        const SteamNetworkingConfigValue_t* pOptions) = 0;
  ...
  virtual HSteamNetConnection ConnectP2P(const SteamNetworkingIdentity& identityRemote,
  ...
  ///   (Which is a member of SteamNetConnectionStatusChangedCallback_t -- but see WARNINGS
  ///   below!!!!)
  ///
  ...
  virtual EResult SendMessageToConnection(HSteamNetConnection hConn, const void* pData,
  ...
  /// same sizes --- see SendMessageToConnection for on this subtle difference from a stream
  /// socket).
  ///
  ...
  virtual int ReceiveMessagesOnConnection(HSteamNetConnection        hConn,
                                          SteamNetworkingMessage_t** ppOutMessages,
                                          int                        nMaxMessages) = 0;

  /// Returns basic information about the high-level state of the connection.
  virtual bool GetConnectionInfo(HSteamNetConnection       hConn,
                                 SteamNetConnectionInfo_t* pInfo) = 0;
  ...
  virtual EResult
  GetConnectionRealTimeStatus(HSteamNetConnection                 hConn,
  ...
  virtual int GetDetailedConnectionStatus(HSteamNetConnection hConn, char* pszBuf,
                                          int cbBuf) = 0;

  /// Returns local IP and port that a listen socket created using CreateListenSocketIP is bound
  /// to.
  ///
  /// An IPv6 address of ::0 means "any IPv4 or IPv6"
  /// An IPv6 address of ::ffff:0000:0000 means "any IPv4"
  virtual bool GetListenSocketAddress(HSteamListenSocket     hSocket,
                                      SteamNetworkingIPAddr* address) = 0;
  ...
  /// will be posted immediately.  After this, if you close either connection, the other
  /// connection will receive a callback, exactly as if they were communicating over the
  /// network.  You must close *both* sides in order to fully clean up the resources!
  ///
  /// By default, internal buffers are used, completely bypassing the network, the chopping up
  /// of messages into packets, encryption, copying the payload, etc.  This means that loopback
  /// packets, by default, will not simulate lag or loss.  Passing true for bUseNetworkLoopback
  /// will cause the socket pair to send packets through the local network loopback device
  /// (127.0.0.1) on ephemeral ports.  Fake lag and loss are supported in this case, and CPU
  /// time is expended to encrypt and decrypt.
  ///
  ...
  virtual bool CreateSocketPair(HSteamNetConnection* pOutConnection1,
  ...
  virtual EResult ConfigureConnectionLanes(HSteamNetConnection hConn, int nNumLanes,
  ...
  /// E.g. on Steam, this is the user's SteamID, or for the gameserver interface, the SteamID
  /// assigned to the gameserver.  Returns false and sets the result to an invalid identity if
  /// we don't know our identity yet.  (E.g. GameServer has not logged in.  On Steam, the user
  /// will know their SteamID even if they are not signed into Steam.)
  ...
  virtual ESteamNetworkingAvailability
  GetAuthenticationStatus(SteamNetAuthenticationStatus_t* pDetails) = 0;
  ...
  virtual bool SetConnectionPollGroup(HSteamNetConnection hConn,
  ...
  /// other connections.)
  virtual int ReceiveMessagesOnPollGroup(HSteamNetPollGroup         hPollGroup,
                                         SteamNetworkingMessage_t** ppOutMessages,
                                         int                        nMaxMessages) = 0;
  ...
  /// attempt will fail!  If you are not issuing your own tickets, then to connect to a
  /// dedicated server via SDR in auto-ticket mode, use ConnectP2P.  (The server must be
  /// configured to allow this type of connection by listening using CreateListenSocketP2P.)
  ///
  /// You may wonder why tickets are stored in a cache, instead of simply being passed as an
  /// argument here.  The reason is to make reconnection to a gameserver robust, even if the
  /// client computer loses connection to Steam or the central backend, or the app is restarted
  /// or crashes, etc.
  ///
  /// If you use this, you probably want to call ISteamNetworkingUtils::InitRelayNetworkAccess()
  /// when your app initializes
  ...
  /// setting the options "immediately" after creation.
  virtual HSteamNetConnection
  ConnectToHostedDedicatedServer(const SteamNetworkingIdentity& identityTarget,
                                 int nRemoteVirtualPort, int nOptions,
                                 const SteamNetworkingConfigValue_t* pOptions) = 0;
  ...
  /// SteamDatagramGameCoordinatorServerLogin, so if possible, it's preferred to use
  /// GetGameCoordinatorServerLogin to send this info to your game coordinator service, and also
  /// login securely at the same time.
  ///
  ...
  /// setting the options "immediately" after creation.
  virtual HSteamListenSocket
  CreateHostedDedicatedServerListenSocket(int nLocalVirtualPort, int nOptions,
                                          const SteamNetworkingConfigValue_t* pOptions) = 0;
  ...
  /// not have a cert.  (You will need to enable insecure dev login in
  /// SteamDatagram_ParseHostedServerLogin.) Otherwise, you will need a signed cert.
  ///
  /// NOTE: The routing blob returned here is not encrypted.  Send it to your backend
  ///       and don't share it directly with clients.
  virtual EResult
  GetGameCoordinatorServerLogin(SteamDatagramGameCoordinatorServerLogin* pLoginInfo,
  ...
  /// setting the options "immediately" after creation.
  virtual HSteamNetConnection
  ConnectP2PCustomSignaling(ISteamNetworkingConnectionSignaling* pSignaling,
  ...
  //
  // Certificate provision by the application.  On Steam, we normally handle all this
  // automatically and you will not need to use these advanced functions.
  //

  /// Get blob that describes a certificate request.  You can send this to your game
  /// coordinator. Upon entry, *pcbBlob should contain the size of the buffer.  On successful
  /// exit, it will return the number of bytes that were populated.  You can pass pBlob=NULL to
  /// query for the required size.  (512 bytes is a conservative estimate.)
  ///
  /// Pass this blob to your game coordinator and call SteamDatagram_CreateCert.
  virtual bool GetCertificateRequest(int* pcbBlob, void* pBlob,
                                     SteamNetworkingErrMsg& errMsg) = 0;

  /// Set the certificate.  The certificate blob should be the output of
  /// SteamDatagram_CreateCert.
  virtual bool SetCertificate(const void* pCertificate, int cbCertificate,
                              SteamNetworkingErrMsg& errMsg) = 0;
  ...
  virtual HSteamListenSocket
  CreateListenSocketP2PFakeIP(int idxFakePort, int nOptions,
                              const SteamNetworkingConfigValue_t* pOptions) = 0;
  ...
  virtual EResult GetRemoteFakeIPForConnection(HSteamNetConnection    hConn,
                                               SteamNetworkingIPAddr* pOutAddr) = 0;
...
STEAM_DEFINE_GAMESERVER_INTERFACE_ACCESSOR(ISteamNetworkingSockets*,
                                           SteamGameServerNetworkingSockets_SteamAPI,
                                           STEAMNETWORKINGSOCKETS_INTERFACE_VERSION);
...
///   k_ESteamNetworkingConnectionState_ClosedByPeer.  m_info.m_eEndReason and
///   m_info.m_szEndDebug will have for more details. NOTE: upon receiving this callback, you
///   must still destroy the connection using ISteamNetworkingSockets::CloseConnection to free
///   up local resources.  (The details passed to the function are not used in this case, since
...
///
/// Also note that callbacks will be posted when connections are created and destroyed by your
/// own API calls.
