...
bool player_record_t::valid( ) const {
	...
	const float lerp = std::fmaxf( cl_interp->m_value.m_float_value, cl_interp_ratio->m_value.m_float_value / cl_updaterate->m_value.m_float_value );
	float correct = lerp;
		...
		if ( player->m_sim_time < target->sim_time( ) ) {
			...
			if ( g_interfaces.m_engine->get_player_info( target->entindex(), &info ) )
				if ( !player->m_records.empty( ) ) {
					...
					new_record.vel = ( new_record.origin - player->m_records[ 0 ]->origin ) * ( 1.f / TICKS_TO_TIME( new_record.m_lag ) );
					...
					if ( new_record.vel.length( ) < 5.f || new_record.origin == player->pred_origin ) 						{
						...
						//if ( ( turn > 0 && turn > player->m_records[ 0 ]->dir ) || ( turn < 0 && turn < player->m_records[ 0 ]->dir ) ) {
							new_record.dir = ApproachAngle(
								turn,
						...
						//else new_record.dir = turn;
							...
							//if ( new_record.on_ground && player->m_records.size() > 3 ) {
								...
								new_record.move_data = g_movement.estimate_walking_dir( new_record.vel, player->m_records[ 0 ]->vel, new_record.eye_angle, new_record.origin );
							...
							//	}
							//	//turn = fminf( max_turn, fmaxf( -max_turn, turn) );
							//	//if ( ( turn > 0 && turn > player->m_records[ 0 ]->ground_dir ) || ( turn < 0 && turn < player->m_records[ 0 ]->ground_dir ) ) {
							//	new_record.ground_dir = new_record.ground_dir / count;
							//}
							//else
						...
						//new_record.ground_dir = ApproachAngle( dir_turning( new_record.vel, player->m_records[ 0 ]->vel ) / new_record.m_lag,
						//	player->m_records[ 0 ]->ground_dir,
						//	( fmaxf( fabsf( dir ) * g_interfaces.m_global_vars->m_interval_per_tick, 720.f * ( g_interfaces.m_global_vars->m_interval_per_tick * g_interfaces.m_global_vars->m_interval_per_tick ))) );
					}
					g_movement.setup_mv( new_record.vel, player->player, g_cl.m_local->entindex( ) );
