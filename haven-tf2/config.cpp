        ...
        for (const auto& tab : tabs)
        {
            ...
            for (const auto& groupbox : groupboxes)
            {
                ...
                for (auto& control : controls)
                {
                    ...
                    this->config_controls_.emplace_back(control, window->m_name, tab->m_name, groupbox->m_name);
                }
            }
        }
    }
...
void c_config::save(const char* file_name) const
{
    try
    {
        ...
        for (auto& config_control : this->config_controls_)
        {
            ...
            auto& var = json[config_control.m_window_name][config_control.m_tab_name][config_control.m_groupbox_name]
                            [control->m_name];
