...
enum ELobbyType {
  ...
  k_ELobbyTypeInvisible =
      3, // returned by search, but not visible to other friends
         //    useful if you want a user in two lobbies, for example matching groups together
         //	  a user can be in only one regular lobby, and up to two invisible lobbies
  k_ELobbyTypePrivateUnique =
      4, // private, unique and does not delete when empty - only one of these may exist per
         // unique keypair set can only create from webapi
};
...
enum ELobbyDistanceFilter {
  ...
  k_ELobbyDistanceFilterFar, // for games that don't have many latency requirements, will return
                             // lobbies about half-way around the globe
  k_ELobbyDistanceFilterWorldwide, // no filtering, will match lobbies as far as India to NY
                                   // (not recommended, expect multiple seconds of latency
                                   // between the clients)
};
...
public:
  ...
  // *punFlags specify whether the game server was stored as an explicit favorite or in the
  // history of connections *pRTime32LastPlayedOnServer is filled in the with the Unix time the
  // favorite was added
  virtual bool GetFavoriteGame(int iGame, AppId_t* pnAppID, uint32* pnIP, uint16* pnConnPort,
                               uint16* pnQueryPort, uint32* punFlags,
                               uint32* pRTime32LastPlayedOnServer) = 0;

  // adds the game server to the local list; updates the time played of the server if it already
  // exists in the list
  virtual int AddFavoriteGame(AppId_t nAppID, uint32 nIP, uint16 nConnPort, uint16 nQueryPort,
                              uint32 unFlags, uint32 rTime32LastPlayedOnServer) = 0;

  // removes the game server from the local storage; returns true if one was removed
  virtual bool RemoveFavoriteGame(AppId_t nAppID, uint32 nIP, uint16 nConnPort,
                                  uint16 nQueryPort, uint32 unFlags) = 0;
  ...
  // results will be returned by LobbyMatchList_t callback & call result, with the number of
  // lobbies found this will never return lobbies that are full to add more filter, the filter
  // calls below need to be call before each and every RequestLobbyList() call use the
  // CCallResult<> object in steam_api.h to match the SteamAPICall_t call result to a function
  // in an object, e.g.
  /*
          class CMyLobbyListManager
          {
                  ...
                  void FindLobbies()
                  {
                          // SteamMatchmaking()->AddRequestLobbyListFilter*() functions would be
     called here, before RequestLobbyList() SteamAPICall_t hSteamAPICall =
     SteamMatchmaking()->RequestLobbyList(); m_CallResultLobbyMatchList.Set( hSteamAPICall,
     this, &CMyLobbyListManager::OnLobbyMatchList );
                  }
  ...
  */
  ...
  virtual void AddRequestLobbyListNumericalFilter(const char* pchKeyToMatch, int nValueToMatch,
                                                  ELobbyComparison eComparisonType)         = 0;
  // returns results closest to the specified value. Multiple near filters can be added, with
  // early filters taking precedence
  virtual void AddRequestLobbyListNearValueFilter(const char* pchKeyToMatch,
                                                  int         nValueToBeCloseTo)                    = 0;
  // returns only lobbies with the specified number of slots available
  virtual void AddRequestLobbyListFilterSlotsAvailable(int nSlotsAvailable)                 = 0;
  // sets the distance for which we should search for lobbies (based on users IP address to
  // location map on the Steam backed)
  virtual void AddRequestLobbyListDistanceFilter(ELobbyDistanceFilter eLobbyDistanceFilter) = 0;
  // sets how many results to return, the lower the count the faster it is to download the lobby
  // results & details to the client
  ...
  // If private, then the lobby will not be returned by any RequestLobbyList() call; the
  // CSteamID of the lobby will need to be communicated via game channels or via
  // InviteUserToLobby() this is an asynchronous request results will be returned by
  // LobbyCreated_t callback and call result; lobby is joined & ready to use at this point a
  // LobbyEnter_t callback will also be received (since the local user is joining their own
  // lobby)
  ...
  // this is an asynchronous request
  // results will be returned by LobbyEnter_t callback & call result, check
  // m_EChatRoomEnterResponse to see if was successful lobby metadata is available to use
  // immediately on this call completing
  ...
  // if the other user clicks the join link, a GameLobbyJoinRequested_t will be posted if the
  // user is in-game, or if the game isn't running yet the game will be launched with the
  ...
  // persona information for other lobby members (name, avatar, etc.) will be asynchronously
  // received and accessible via ISteamFriends interface

  // returns the number of users in the specified lobby
  virtual int      GetNumLobbyMembers(CSteamID steamIDLobby)                 = 0;
  // returns the CSteamID of a user in the lobby
  // iMember is of range [0,GetNumLobbyMembers())
  // note that the current user must be in a lobby to retrieve CSteamIDs of other users in that
  // lobby
  ...
  // each user in the lobby will be broadcast this new value, and any new users joining will
  // receive any existing data this can be used to set lobby names, map, etc. to reset a key,
  // just set it to "" other users in the lobby will receive notification of the lobby data
  // change via a LobbyDataUpdate_t callback
  virtual bool        SetLobbyData(CSteamID steamIDLobby, const char* pchKey,
  ...
  virtual bool GetLobbyDataByIndex(CSteamID steamIDLobby, int iLobbyData, char* pchKey,
                                   int cchKeyBufferSize, char* pchValue,
                                   int cchValueBufferSize) = 0;

  // removes a metadata key from the lobby
  virtual bool DeleteLobbyData(CSteamID steamIDLobby, const char* pchKey) = 0;

  // Gets per-user metadata for someone in this lobby
  virtual const char* GetLobbyMemberData(CSteamID steamIDLobby, CSteamID steamIDUser,
                                         const char* pchKey)   = 0;
  // Sets per-user metadata (for the local user implicitly)
  virtual void        SetLobbyMemberData(CSteamID steamIDLobby, const char* pchKey,
                                         const char* pchValue) = 0;
  ...
  // if pvMsgBody is text, cubMsgBody should be strlen( text ) + 1, to include the null
  // terminator
  virtual bool SendLobbyChatMsg(CSteamID steamIDLobby, const void* pvMsgBody,
                                int cubMsgBody)                                = 0;
  ...
  virtual int  GetLobbyChatEntry(CSteamID steamIDLobby, int iChatID,
                                 STEAM_OUT_STRUCT() CSteamID* pSteamIDUser, void* pvData,
                                 int cubData, EChatEntryType* peChatEntryType) = 0;
  ...
  // either the IP/Port or the steamID of the game server has to be valid, depending on how you
  // want the clients to be able to connect
  virtual void SetLobbyGameServer(CSteamID steamIDLobby, uint32 unGameServerIP,
                                  uint16 unGameServerPort, CSteamID steamIDGameServer) = 0;
  // returns the details of a game server set in a lobby - returns false if there is no game
  // server set, or that lobby doesn't exist
  virtual bool GetLobbyGameServer(CSteamID steamIDLobby, uint32* punGameServerIP,
  ...
  // returns the current limit on the # of users who can join the lobby; returns 0 if no limit
  // is defined
  virtual int  GetLobbyMemberLimit(CSteamID steamIDLobby)                  = 0;

  // updates which type of lobby it is
  // only lobbies that are k_ELobbyTypePublic or k_ELobbyTypeInvisible, and are set to joinable,
  // will be returned by RequestLobbyList() calls
  ...
  // there always one lobby owner - if the current owner leaves, another user will become the
  // owner it is possible (bur rare) to join a lobby just as the owner is leaving, thus entering
  // a lobby with self as the owner
...
public:
  ...
  virtual void RefreshComplete(HServerListRequest         hRequest,
                               EMatchMakingServerResponse response) = 0;
};
...
public:
  // Request a new list of servers of a particular type.  These calls each correspond to one of
  // the EMatchMakingType values. Each call allocates a new asynchronous request object. Request
  // object must be released by calling ReleaseRequest( hServerListRequest )
  virtual HServerListRequest RequestInternetServerList(
      AppId_t iApp, STEAM_ARRAY_COUNT(nFilters) MatchMakingKeyValuePair_t** ppchFilters,
      uint32 nFilters, ISteamMatchmakingServerListResponse* pRequestServersResponse) = 0;
  virtual HServerListRequest
  ...
  // Releases the asynchronous request object and cancels any pending query on it if there's a
  // pending query in progress. RefreshComplete callback is not posted when request is released.
  virtual void ReleaseRequest(HServerListRequest hServerListRequest) = 0;

  /* the filter operation codes that go in the key part of MatchMakingKeyValuePair_t should be
     one of these:
     ...
     (ISteamGameServer::SetGameData) contains all of the specified strings.  The value field is
     a comma-delimited list of strings to match. "gamedataor"
                  - Server passes the filter if the server's game data
     (ISteamGameServer::SetGameData) contains at least one of the specified strings.  The value
     field is a comma-delimited list of strings to match. "gamedatanor"
                  - Server passes the filter if the server's game data
     (ISteamGameServer::SetGameData) does not contain any of the specified strings.  The value
     field is a comma-delimited list of strings to check. "gametagsand"
                  - Server passes the filter if the server's game tags
     (ISteamGameServer::SetGameTags) contains all of the specified strings.  The value field is
     a comma-delimited list of strings to check. "gametagsnor"
                  - Server passes the filter if the server's game tags
     (ISteamGameServer::SetGameTags) does not contain any of the specified strings.  The value
     field is a comma-delimited list of strings to check. "and" (x1 && x2 && ... && xn) "or" (x1
     || x2 || ... || xn) "nand" !(x1 && x2 && ... && xn) "nor" !(x1 || x2 || ... || xn)
                  - Performs Boolean operation on the following filters.  The operand to this
     filter specifies the "size" of the Boolean inputs to the operation, in Key/value pairs.
     (The keyvalue pairs must immediately follow, i.e. this is a prefix logical operator
     notation.) In the simplest case where Boolean expressions are not nested, this is simply
                  the number of operands.

                  For example, to match servers on a particular map or with a particular tag,
     would would use these filters.
     ...
     entire "length" of its operands, not the number of immediate children.
                          ...
                          ( server.map == "cp_dustbowl" || ( server.gametags.contains("payload")
     && !server.gametags.contains("payloadrace") ) ) "or", "4" "map", "cp_dustbowl" "and", "2"
                          "gametagsand", "payload"
          ...
          "addr"
                  - Server passes the filter if the server's query address matches the specified
     IP or IP:port. "gameaddr"
                  - Server passes the filter if the server's game address matches the specified
     IP or IP:port.
          ...
          "notfull"
                  - Server passes the filter if the player count is less than the reported max
     player count. "hasplayers"
  ...
  */
  ...
  // Cancel an request which is operation on the given list type.  You should call this to
  // cancel any in-progress requests before destructing a callback object that may have been
  // passed to one of the above list request calls.  Not doing so may result in a crash when a
  // callback occurs on the destructed object. Canceling a query does not release the allocated
  // request handle. The request handle must be released using ReleaseRequest( hRequest )
  ...
  virtual HServerQuery
  PlayerDetails(uint32 unIP, uint16 usPort,
                ISteamMatchmakingPlayersResponse* pRequestServersResponse) = 0;

  // Request the list of rules that the server is running (See ISteamGameServer::SetKeyValue()
  // to set the rules server side)
...
const uint32 k_unFavoriteFlagFavorite =
    0x01; // this game favorite entry is for the favorites list
...
enum EChatMemberStateChange {
  ...
  k_EChatMemberStateChangeDisconnected =
      0x0004,                              // User disconnected without leaving the chat first
...
#define BChatMemberStateChangeRemoved(rgfChatMemberStateChangeFlags)                           \
  (rgfChatMemberStateChangeFlags &                                                             \
   (k_EChatMemberStateChangeDisconnected | k_EChatMemberStateChangeLeft |                      \
...
public:
  ...
  // a keyname and a list of comma separated values: one of which is must be found in order for
  // the match to qualify fails if a search is currently in progress
  virtual EGameSearchErrorCode_t AddGameSearchParams(const char* pchKeyToFind,
                                                     const char* pchValuesToFind) = 0;

  // all players in lobby enter the queue and await a SearchForGameNotificationCallback_t
  // callback. fails if another search is currently in progress if not the owner of the lobby or
  // search already in progress this call fails periodic callbacks will be sent as queue time
  // estimates change
  virtual EGameSearchErrorCode_t SearchForGameWithLobby(CSteamID steamIDLobby, int nPlayerMin,
                                                        int nPlayerMax) = 0;

  // user enter the queue and await a SearchForGameNotificationCallback_t callback. fails if
  // another search is currently in progress periodic callbacks will be sent as queue time
  // estimates change
  ...
  // multiple SearchForGameResultCallback_t will follow as players accept game until the host
  // starts or cancels the game
  ...
  virtual EGameSearchErrorCode_t RetrieveConnectionDetails(CSteamID steamIDHost,
  ...
  virtual EGameSearchErrorCode_t SetGameHostParams(const char* pchKey,
                                                   const char* pchValue) = 0;
  ...
  // RequestPlayersForGameNotificationCallback_t callback will be sent when the search has
  // started multple RequestPlayersForGameResultCallback_t callbacks will follow when players
  // are found
  virtual EGameSearchErrorCode_t RequestPlayersForGame(int nPlayerMin, int nPlayerMax,
                                                       int nMaxTeamSize) = 0;
  ...
  // ( allows host to accept after all players confirm, some confirm, or none confirm. decision
  ...
  // if a set of players has already been sent to host, all players will receive
  // SearchForGameHostFailedToConfirm_t
  virtual EGameSearchErrorCode_t CancelRequestPlayersForGame() = 0;

  // submit a result for one player. does not end the game. ullUniqueGameID continues to
  // describe this game
  virtual EGameSearchErrorCode_t SubmitPlayerResult(uint64          ullUniqueGameID,
...
STEAM_DEFINE_USER_INTERFACE_ACCESSOR(ISteamGameSearch*, SteamGameSearch,
...
public:
  ...
  virtual bool GetBeaconDetails(PartyBeaconID_t ulBeaconID, CSteamID* pSteamIDBeaconOwner,
                                ...
                                STEAM_OUT_STRING_COUNT(cchMetadata) char*      pchMetadata,
                                int                                            cchMetadata)           = 0;
  ...
  virtual SteamAPICall_t CreateBeacon(uint32                      unOpenSlots,
                                      SteamPartyBeaconLocation_t* pBeaconLocation,
                                      const char*                 pchConnectString,
                                      const char*                 pchMetadata) = 0;
  ...
  // Note: The user may already be in-flight to your game, so it's possible they will still
  // connect and try to join your party.
  ...
  // Call this if, for example, someone without a reservation joins your party (eg a friend, or
  // via your own matchmaking system).
  ...
  // Utils
  virtual bool
  GetBeaconLocationData(SteamPartyBeaconLocation_t                     BeaconLocation,
...
STEAM_DEFINE_USER_INTERFACE_ACCESSOR(ISteamParties*, SteamParties,
                                     STEAMPARTIES_INTERFACE_VERSION);

//-----------------------------------------------------------------------------
// Callbacks for ISteamMatchmaking (which go through the regular Steam callback registration
// system)
...
//			the Steam UI will also display a '<user> has invited you to the lobby,
// join?' dialog
//
//			if the user outside a game chooses to join, your game will be launched
// with the parameter "+connect_lobby <64-bit lobby id>", 			or with the
// callback GameLobbyJoinRequested_t if they're already in-game
//-----------------------------------------------------------------------------
...
//			m_EChatRoomEnterResponse will be set to k_EChatRoomEnterResponseSuccess
// on success, 			or a higher value on failure (see enum EChatRoomEnterResponse)
//-----------------------------------------------------------------------------
struct LobbyEnter_t {
...
//			if m_ulSteamIDMember is the steamID of a lobby member, use
// GetLobbyMemberData() to access per-user details 			if m_ulSteamIDMember ==
// m_ulSteamIDLobby, use GetLobbyData() to access lobby metadata
//-----------------------------------------------------------------------------
struct LobbyDataUpdate_t {
  ...
  uint8  m_bSuccess;        // true if we lobby data was successfully changed;
                    // will only be false if RequestLobbyData() was called on a lobby that no
                    // longer exists
};
...
struct LobbyChatUpdate_t {
  ...
  uint64 m_ulSteamIDMakingChange; // Chat member who made the change (different from
                                  // SteamIDUserChange if kicking, muting, etc.) for example, if
                                  // one user kicks another from the lobby, this will be set to
                                  // the id of the user who initiated the kick
...
//			use GetLobbyChatEntry( m_iChatID ) to retrieve the contents of this
// message
//-----------------------------------------------------------------------------
...
//			game behavior is to leave the lobby and connect to the specified game
// server
//-----------------------------------------------------------------------------
struct LobbyGameCreated_t {
  ...
  uint64 m_ulSteamIDGameServer; // the new game server that has been created or found for the
                                // lobby members
...
//			iterate the returned lobbies with GetLobbyByIndex(), from values 0 to
// m_nLobbiesMatching-1
//-----------------------------------------------------------------------------
struct LobbyMatchList_t {
  enum { k_iCallback = k_iSteamMatchmakingCallbacks + 10 };
  uint32 m_nLobbiesMatching; // Number of lobbies that matched search criteria and we have
                             // SteamIDs for
};
...
struct LobbyKicked_t {
  ...
  uint8  m_bKickedDueToDisconnect; // true if you were kicked from the lobby due to the user
                                   // losing connection to Steam (currently always true)
};
...
//			a LobbyEnter_t callback will also be received (since the local user is
...
struct LobbyCreated_t {
  ...
  EResult m_eResult; // k_EResultOK - the lobby was successfully created
                     // k_EResultNoConnection - your Steam client doesn't have a connection to
                     // the back-end k_EResultTimeout - you the message to the Steam servers,
                     // but it didn't respond k_EResultFail - the server responded, but with an
                     // unknown internal error k_EResultAccessDenied - your game isn't set to
                     // allow lobbies, or your client does haven't rights to play the game
...
//			game code should wait until the subsequent LobbyEnter_t callback is
// received
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Callbacks for ISteamGameSearch (which go through the regular Steam callback registration
// system)
...
struct SearchForGameProgressCallback_t {
  ...
  uint64
      m_ullSearchID; // all future callbacks referencing this search will include this Search ID

  EResult  m_eResult; // if search has started this result will be k_EResultOK, any other value
                      // indicates search has failed to start or has terminated
...
// callback will also follow a call from CancelRequestPlayersForGame - m_bSearchInProgress will
// be false
