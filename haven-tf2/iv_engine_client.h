...
public:
    virtual int get_intersecting_surfaces(const model_t* model, const vector& v_center, const float radius,
                                          const bool only_visible_surfaces, surf_info* infos, const int max_infos) = 0;
    ...
    virtual i_material* trace_line_material_and_lighting(const vector& start, const vector& end,
                                                         vector& diffuse_light_color, vector& base_color) = 0;
    ...
    virtual int sentence_group_pick_sequential(int group_index, char* name, int name_buf_len, int sentence_index,
                                               int reset) = 0;
    ...
    virtual void compute_lighting(const vector& pt, const vector* normal, bool clamp, vector& color,
                                  vector* box_colors = NULL) = 0;
    ...
    virtual void debug_draw_phys_collide(const c_phys_collide* collide, i_material* material, matrix_3x4& transform,
