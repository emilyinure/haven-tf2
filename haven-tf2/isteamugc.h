...
enum EUGCMatchingUGCType {
  ...
  k_EUGCMatchingUGCType_All =
      ~0, // @note: will only be valid for CreateQueryUserUGCRequest requests
};

// Different lists of published UGC for a user.
// If the current logged in user is different than the specified user, then some options may not
// be allowed.
enum EUserUGCList {
...
enum EItemUpdateStatus {
  k_EItemUpdateStatusInvalid = 0, // The item update handle was invalid, job might be finished,
                                  // listen too SubmitItemUpdateResult_t
...
enum EItemState {
  ...
  k_EItemStateNeedsUpdate = 8,  // items needs an update. Either because it's not installed yet
                                // or creator updated content
  k_EItemStateDownloading = 16, // item update is currently downloading
  k_EItemStateDownloadPending = 32, // DownloadItem() was called for this item, content isn't
                                    // available until DownloadItemResult_t is fired
};
...
enum EItemPreviewType {
  ...
  k_EItemPreviewType_EnvironmentMap_HorizontalCross =
      3, // standard image file expected - cube map in the layout
         // +---+---+-------+
         // |   |Up |       |
         // +---+---+---+---+
         // | L | F | R | B |
...
struct SteamUGCDetails_t {
  ...
  uint32 m_rtimeAddedToUserList; // time when the user added the published file to their list
                                 // (not always applicable)
  ERemoteStoragePublishedFileVisibility m_eVisibility; // visibility
  bool                                  m_bBanned;     // whether the file was banned
  bool m_bAcceptedForUse; // developer has specifically flagged this item as accepted in the
                          // Workshop
  bool m_bTagsTruncated; // whether the list of tags was too long to be returned in the provided
                         // buffer
  char
      m_rgchTags[k_cchTagListMax]; // comma separated list of all tags associated with this file
...
class ISteamUGC {
public:
  // Query UGC associated with a user. Creator app id or consumer app id must be valid and be
  // set to the current running app. unPage should start at 1.
  virtual UGCQueryHandle_t CreateQueryUserUGCRequest(AccountID_t           unAccountID,
  ...
  // Query for all matching UGC. Creator app id or consumer app id must be valid and be set to
  // the current running app. unPage should start at 1.
  STEAM_FLAT_NAME(CreateQueryAllUGCRequestPage)
  virtual UGCQueryHandle_t
  CreateQueryAllUGCRequest(EUGCQuery           eQueryType,
                           EUGCMatchingUGCType eMatchingeMatchingUGCTypeFileType,
                           AppId_t nCreatorAppID, AppId_t nConsumerAppID, uint32 unPage) = 0;

  // Query for all matching UGC using the new deep paging interface. Creator app id or consumer
  // app id must be valid and be set to the current running app. pchCursor should be set to NULL
  // or "*" to get the first result set.
  ...
  // Query for the details of the given published file ids (the RequestUGCDetails call is
  // deprecated and replaced with this)
  ...
  virtual bool GetQueryUGCPreviewURL(UGCQueryHandle_t handle, uint32 index,
                                     STEAM_OUT_STRING_COUNT(cchURLSize) char* pchURL,
                                     uint32                                   cchURLSize)                                  = 0;
  ...
  virtual bool GetQueryUGCStatistic(UGCQueryHandle_t handle, uint32 index,
                                    EItemStatistic eStatType, uint64* pStatValue)        = 0;
  ...
  virtual bool   GetQueryUGCAdditionalPreview(
        ...
        STEAM_OUT_STRING_COUNT(cchURLSize) char* pchURLOrVideoID, uint32 cchURLSize,
  ...
  virtual bool   GetQueryUGCKeyValueTag(UGCQueryHandle_t handle, uint32 index,
                                        ...
                                        STEAM_OUT_STRING_COUNT(cchKeySize) char* pchKey,
                                        uint32                                   cchKeySize,
                                        STEAM_OUT_STRING_COUNT(cchValueSize) char* pchValue,
                                        uint32 cchValueSize)                       = 0;
  ...
  virtual bool SetReturnAdditionalPreviews(UGCQueryHandle_t handle,
                                           bool             bReturnAdditionalPreviews)              = 0;
  ...
  virtual bool SetTimeCreatedDateRange(UGCQueryHandle_t handle, RTime32 rtStart,
                                       RTime32 rtEnd)                          = 0;
  virtual bool SetTimeUpdatedDateRange(UGCQueryHandle_t handle, RTime32 rtStart,
                                       RTime32 rtEnd)                          = 0;
  virtual bool AddRequiredKeyValueTag(UGCQueryHandle_t handle, const char* pKey,
                                      const char* pValue)                      = 0;
  ...
  virtual SteamAPICall_t
  CreateItem(AppId_t nConsumerAppId,
             EWorkshopFileType
                 eFileType) = 0; // create new item for this app with no content attached yet

  virtual UGCUpdateHandle_t
  StartItemUpdate(AppId_t nConsumerAppId,
                  PublishedFileId_t
                      nPublishedFileID) = 0; // start an UGC item update. Set changed properties
                                             // before commiting update with CommitItemUpdate()

  virtual bool SetItemTitle(UGCUpdateHandle_t handle,
                            const char*       pchTitle) = 0; // change the title of an UGC item
  virtual bool
  SetItemDescription(UGCUpdateHandle_t handle,
                     const char* pchDescription) = 0; // change the description of an UGC item
  virtual bool
  SetItemUpdateLanguage(UGCUpdateHandle_t handle,
                        const char* pchLanguage) = 0; // specify the language of the title or
                                                      // description that will be set
  virtual bool
               SetItemMetadata(UGCUpdateHandle_t handle,
                               const char* pchMetaData) = 0; // change the metadata of an UGC item (max =
                                                             // k_cchDeveloperMetadataMax)
  virtual bool SetItemVisibility(UGCUpdateHandle_t handle,
                                 ERemoteStoragePublishedFileVisibility
                                     eVisibility) = 0; // change the visibility of an UGC item
  virtual bool
               SetItemTags(UGCUpdateHandle_t              updateHandle,
  ...
  virtual bool
               SetItemPreview(UGCUpdateHandle_t handle,
                              const char* pszPreviewFile) = 0; //  change preview image file for this item.
                                                  //  pszPreviewFile points to local image file,
                                                  //  which must be under 1MB in size
  virtual bool SetAllowLegacyUpload(
      UGCUpdateHandle_t handle,
      bool bAllowLegacyUpload) = 0; //  use legacy upload for a single small file. The parameter
                                    //  to SetItemContent() should either be a directory with
                                    //  one file or the full path to the file.  The file must
                                    //  also be less than 10MB in size.
  virtual bool RemoveAllItemKeyValueTags(
      UGCUpdateHandle_t handle) = 0; // remove all existing key-value tags (you can add new ones
                                     // via the AddItemKeyValueTag function)
  virtual bool RemoveItemKeyValueTags(
      UGCUpdateHandle_t handle,
      const char* pchKey) = 0; // remove any existing key-value tags with the specified key
  virtual bool
               AddItemKeyValueTag(UGCUpdateHandle_t handle, const char* pchKey,
                                  const char* pchValue) = 0; // add new key-value tags for the item. Note
                                                // that there can be multiple values for a tag.
  virtual bool AddItemPreviewFile(
      UGCUpdateHandle_t handle, const char* pszPreviewFile,
      EItemPreviewType type) = 0; //  add preview file for this item. pszPreviewFile points to
                                  //  local file, which must be under 1MB in size
  virtual bool
               AddItemPreviewVideo(UGCUpdateHandle_t handle,
  ...
  virtual bool UpdateItemPreviewFile(
      ...
      const char* pszPreviewFile) = 0; //  updates an existing preview file for this item.
                                       //  pszPreviewFile points to local file, which must be
                                       //  under 1MB in size
  ...
  virtual bool AddContentDescriptor(UGCUpdateHandle_t       handle,
                                    EUGCContentDescriptorID descid)    = 0;
  virtual bool RemoveContentDescriptor(UGCUpdateHandle_t       handle,
                                       EUGCContentDescriptorID descid) = 0;
  ...
  virtual SteamAPICall_t AddItemToFavorites(AppId_t           nAppId,
                                            PublishedFileId_t nPublishedFileID) = 0;
  STEAM_CALL_RESULT(UserFavoriteItemsListChanged_t)
  virtual SteamAPICall_t RemoveItemFromFavorites(AppId_t           nAppId,
                                                 PublishedFileId_t nPublishedFileID) = 0;
  ...
  virtual SteamAPICall_t
  UnsubscribeItem(PublishedFileId_t nPublishedFileID) = 0; // unsubscribe from this item, will
                                                           // be uninstalled after game quits
  virtual uint32 GetNumSubscribedItems()              = 0; // number of subscribed items
  virtual uint32
  GetSubscribedItems(PublishedFileId_t* pvecPublishedFileID,
  ...
  // k_EItemStateInstalled set if k_EItemStateLegacyItem is set, pchFolder contains the path to
  // the legacy file itself (not a folder)
  virtual bool GetItemInstallInfo(PublishedFileId_t nPublishedFileID, uint64* punSizeOnDisk,
                                  STEAM_OUT_STRING_COUNT(cchFolderSize) char* pchFolder,
                                  uint32 cchFolderSize, uint32* punTimeStamp) = 0;

  // get info about pending update for items that have k_EItemStateNeedsUpdate set.
  // punBytesTotal will be valid after download started once
  virtual bool GetItemDownloadInfo(PublishedFileId_t nPublishedFileID,
                                   uint64* punBytesDownloaded, uint64* punBytesTotal) = 0;

  // download new or update already installed item. If function returns true, wait for
  // DownloadItemResult_t. If the item is already installed, then files on disk should not be
  // used until callback received. If item is not subscribed to, it will be cached for some
  // time. If bHighPriority is set, any other item download will be suspended and this item
  // downloaded ASAP.
  ...
  // This is helpful if you want to support multiple game servers running out of the same
  // install folder
  virtual bool BInitWorkshopForGameServer(DepotId_t   unWorkshopDepotID,
                                          const char* pszFolder) = 0;

  // SuspendDownloads( true ) will suspend all workshop downloads until SuspendDownloads( false
  // ) is called or the game ends
  ...
  virtual SteamAPICall_t AddAppDependency(PublishedFileId_t nPublishedFileID,
                                          AppId_t           nAppID) = 0;
  ...
  virtual SteamAPICall_t RemoveAppDependency(PublishedFileId_t nPublishedFileID,
                                             AppId_t           nAppID) = 0;
  // request app dependencies. note that whatever callback you register for
  // GetAppDependenciesResult_t may be called multiple times until all app dependencies have
  // been returned
  ...
  // Show the app's latest Workshop EULA to the user in an overlay window, where they can accept
  // it or not
  virtual bool ShowWorkshopEULA() = 0;
  // Retrieve information related to the user's acceptance or not of the app's specific Workshop
  // EULA
...
STEAM_DEFINE_GAMESERVER_INTERFACE_ACCESSOR(ISteamUGC*, SteamGameServerUGC,
...
struct SteamUGCQueryCompleted_t {
  ...
  char m_rgchNextCursor[k_cchPublishedFileURLMax]; // If a paging cursor was used, then this
                                                   // will be the next cursor to get the next
                                                   // result set.
};
