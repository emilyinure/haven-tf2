...
void c_combobox::paint()
{
    ...
    this->dropdown_area_ = {this->m_area.m_x + (this->m_area.m_w - (this->m_area.m_w * 0.5f)), this->m_area.m_y + 2,
                            this->m_area.m_w * 0.5f, 14};

    // set our item area.
    this->item_area_ = {this->m_area.m_x + (this->m_area.m_w - (this->m_area.m_w * 0.5f)), this->m_area.m_y + 2,
                        this->m_area.m_w * 0.5f, this->items_.size() * 13.f};
    ...
    // current value.
    if (!this->open_)
        g_ui.m_draw.text(g_render.m_fonts.menu.main, {this->dropdown_area_.m_x + 2, this->dropdown_area_.m_y},
                         this->items_[this->m_selected_index], color(255, 255, 255, 100 * g_ui.m_fade),
                         e_text_alignment::text_align_left);

    g_ui.m_theme.m_a = 100 * g_ui.m_fade;
    // title.
    g_ui.m_draw.text(g_render.m_fonts.menu.main, {this->m_area.m_x, this->m_area.m_y + 2}, this->m_name, g_ui.m_theme,
                     e_text_alignment::text_align_left);
    ...
    for (auto i = 0; i < this->items_.size(); i++)
    {
        ...
        const bool hovering =
            g_input.hovering({this->item_area_.m_x + 2, this->item_area_.m_y + 2 + (i * 13), this->item_area_.m_w, 13});
        g_ui.m_draw.text(g_ui.m_draw.fonts.main, {this->item_area_.m_x + 2, this->item_area_.m_y + (i * 13)}, item,
                         this->m_selected_index == i ? g_ui.m_theme
...
void c_combobox::update()
{
    ...
    for (auto i = 0; i < this->items_.size(); i++)
    {
        ...
        if (g_input.hovering(
                {this->item_area_.m_x + 2, this->item_area_.m_y + 2 + (i * 13), this->item_area_.m_w, 13}) &&
