...
typedef struct ScePadTriggerEffectFeedbackParam {
  ...
  uint8_t strength; /*E strength that the motor arm pushes back target trigger(0~8 (0: Same as
                       Off mode)). */
  uint8_t padding[46];
...
typedef struct ScePadTriggerEffectWeaponParam {
  ...
  uint8_t endPosition;   /*E position where the stiffness of trigger finish
                            changing(startPosition+1~8). */
...
/**
 *E
 *  @brief parameter for setting the trigger effect to
 *ScePadTriggerEffectMultiplePositionFeedbackParam mode. Multi Position Feedback Mode: The motor
 *arm pushes back trigger. Trigger obtains specified stiffness at each control point.
 **/
typedef struct ScePadTriggerEffectMultiplePositionFeedbackParam {
  uint8_t
      strength[SCE_PAD_TRIGGER_EFFECT_CONTROL_POINT_NUM]; /*E strength that the motor arm pushes
                                                           * back target trigger at position(0~8
                                                           * (0: Same as Off mode)). strength[0]
                                                           * means strength of motor arm at
                                                           * position0. strength[1] means
                                                           * strength of motor arm at position1.
                                                           *  ...
                                                           * */
  uint8_t padding[38];
 ...
 *         Slope Feedback Mode: The motor arm pushes back trigger between two spedified control
 *points. Stiffness of the trigger is changing depending on the set place.
 **/
typedef struct ScePadTriggerEffectSlopeFeedbackParam {

  uint8_t startPosition; /*E position where the strength of target trigger start
                            changing(0~endPosition). */
  uint8_t endPosition;   /*E position where the strength of target trigger finish
                            changing(startPosition+1~9). */
 ...
 *                                        Trigger vibrates specified amplitude at each control
 *point.
 **/
typedef struct ScePadTriggerEffectMultiplePositionVibrationParam {
  ...
  uint8_t amplitude
      [SCE_PAD_TRIGGER_EFFECT_CONTROL_POINT_NUM]; /*E vibration amplitude at position(0~8 (0:
                                                   * Same as Off mode)). amplitude[0] means
                                                   * amplitude of vibration at position0.
                                                   *  amplitude[1] means amplitude of vibration
                                                   * at position1.
                                                   *  ...
                                                   * */
  uint8_t padding[37];
...
typedef struct ScePadTriggerEffectParam {
  ...
  ScePadTriggerEffectCommand command
      [SCE_PAD_TRIGGER_EFFECT_TRIGGER_NUM]; /*E
                                             * command[SCE_PAD_TRIGGER_EFFECT_PARAM_INDEX_FOR_L2]
                                             * is for L2 trigger setting and
                                             * param[SCE_PAD_TRIGGER_EFFECT_PARAM_INDEX_FOR_R2]
                                             * is for R2 trgger setting.
                                             * */
} ScePadTriggerEffectParam;

#if defined(__cplusplus) && __cplusplus >= 201103L
static_assert(sizeof(ScePadTriggerEffectParam) == 120,
              "ScePadTriggerEffectParam has incorrect size");
#endif
