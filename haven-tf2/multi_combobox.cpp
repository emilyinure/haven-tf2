...
void c_multi_combobox::paint() {
  ...
  this->item_area_ = {this->m_area.m_x + (this->m_area.m_w - (this->m_area.m_w * 0.5f)),
                      this->m_area.m_y + 2, this->m_area.m_w * 0.5f,
                      this->m_items.size() * 14.f};
  ...
  sprintf_s(this->display_buffer_,
            selected_items == 1 ? "%i item selected" : "%i items selected", selected_items);

  // current value.
  g_ui.m_draw.text(g_render.m_fonts.menu.main,
                   {this->dropdown_area_.m_x + 2, this->dropdown_area_.m_y},
                   this->display_buffer_, {255, 255, 255}, e_text_alignment::text_align_left);

  // title.
  g_ui.m_draw.text(g_render.m_fonts.menu.main, {this->m_area.m_x, this->m_area.m_y + 2},
                   this->m_name, {255, 255, 255}, e_text_alignment::text_align_left);
  ...
  for (auto i = 0; i < this->m_items.size(); i++) {
    ...
    g_ui.m_draw.text(g_ui.m_draw.fonts.main,
                     {this->item_area_.m_x + 2, this->item_area_.m_y + (i * 14)}, item.first,
                     item.second ? g_ui.m_theme : color(255, 255, 255),
...
void c_multi_combobox::update() {
  ...
  for (auto i = 0; i < this->m_items.size(); i++) {
    ...
    if (g_input.hovering({this->item_area_.m_x + 2, this->item_area_.m_y + 2 + (i * 14),
                          this->item_area_.m_w, 14}) &&
        g_input.key_pressed(VK_LBUTTON))
      item.second = !item.second;
  }
}

void c_multi_combobox::add_item(const char* item) { this->m_items.emplace_back(item, false); }
