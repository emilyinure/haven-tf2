...
vector c_base_entity::eye_pos() { return m_vec_origin() + m_view_offset(); }
...
vector c_base_entity::get_abs_velocity() { return this->get<vector>(472); }
...
box_t c_base_entity::get_bounding_box() {
  ...
  if (!math::world_to_screen(mins, mins_screen) || !math::world_to_screen(maxs, maxs_screen) ||
...
bool c_base_entity::compute_hitbox_surrounding_box(vector* world_mins, vector* world_maxs) {
  static auto compute_hitbox_surrounding_box =
      reinterpret_cast<compute_hitbox_surrounding_box_fn>(
          g_offsets.m_sigs.compute_hitbox_surrounding_box);
...
c_base_entity* c_base_player::calculate_ground() {
  ...
  auto try_touch_ground_in_quadrants = [](c_base_player* player, vector minsSrc, vector maxsSrc,
  ...
  if (!pm.m_entity || pm.m_plane.normal[2] < 0.7) {
    ...
    // actually stand on
    try_touch_ground_in_quadrants(this, player_mins, player_maxs, bump_origin, point,
...
CBoneCacheHandler* c_base_player::bone_cache() {
  ...
  static auto func = g_modules.get("client.dll")
                         .get_sig("55 8B EC 83 EC ? 56 8B F1 57 FF B6 ? ? ? ?")
                         .as<oSetAbsOrigin>();
...
bool c_base_player::is_valid(c_base_player* local, const bool team_check,
                             const bool dormant_check, const bool alive_check) {
  if (!this)
    return false;
  if (auto client_class = this->get_client_class();
      !client_class || client_class->m_class_id != CTFPlayer)
    return false;
...
short& c_base_weapon::item_index() {
  // g_netvars.m_offsets.dt_base_weapon.m_item_definition_index = g_netvars.get(
  // "DT_BaseCombatWeapon", "m_iItemDefinitionIndex" ).m_offset;
...
bool c_base_weapon::can_perform_backstab_against_target(c_base_player* player) {
  static auto can_perform_backstab_against_target_fn =
      g_modules.get("client.dll")
          .get_sig(
              "48 89 5C 24 08 48 89 6C 24 10 48 89 74 24 18 57 48 83 EC 40 0F 29 74 24 30 48")
          .m_ptr;
  return reinterpret_cast<bool (*)(void*, c_base_player*)>(
      can_perform_backstab_against_target_fn)(this, player);
}
...
bool c_tf_player_shared::in_cond(e_tf_cond cond) {
  static auto in_cond = (in_cond_fn)g_offsets.m_sigs.in_cond;
