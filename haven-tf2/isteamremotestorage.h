...
//-----------------------------------------------------------------------------
// Purpose: Structure that contains an array of const char * strings and the number of those
// strings
//-----------------------------------------------------------------------------
...
enum EWorkshopFileType {
  ...
  k_EWorkshopFileTypeMerch =
      11, // Workshop merchandise meant to be voted on for the purpose of being sold
...
enum EUGCReadAction {
  // Keeps the file handle open unless the last byte is read.  You can use this when reading
  // large files (over 100MB) in sequential chunks. If the last byte is read, this will behave
  // the same as k_EUGCRead_Close.  Otherwise, it behaves the same as
  // k_EUGCRead_ContinueReading. This value maintains the same behavior as before the
  // EUGCReadAction parameter was introduced.
  k_EUGCRead_ContinueReadingUntilFinished = 0,

  // Keeps the file handle open.  Use this when using UGCRead to seek to different parts of the
  // file. When you are done seeking around the file, make a final call with k_EUGCRead_Close to
  // close it.
...
public:
  ...
  virtual SteamAPICall_t FileWriteAsync(const char* pchFile, const void* pvData,
                                        uint32 cubData) = 0;

  STEAM_CALL_RESULT(RemoteStorageFileReadAsyncComplete_t)
  virtual SteamAPICall_t FileReadAsync(const char* pchFile, uint32 nOffset,
                                       uint32 cubToRead)         = 0;
  virtual bool           FileReadAsyncComplete(SteamAPICall_t hReadCall, void* pvBuffer,
                                               uint32 cubToRead) = 0;
  ...
  virtual bool           SetSyncPlatforms(const char*            pchFile,
                                          ERemoteStoragePlatform eRemoteStoragePlatform) = 0;
  ...
  virtual bool FileWriteStreamWriteChunk(UGCFileWriteStreamHandle_t writeHandle,
                                         const void* pvData, int32 cubData)   = 0;
  ...
  // Gets the amount of data downloaded so far for a piece of content. pnBytesExpected can be 0
  // if function returns false or if the transfer hasn't started yet, so be careful to check for
  // that before dividing to get a percentage
  virtual bool GetUGCDownloadProgress(UGCHandle_t hContent, int32* pnBytesDownloaded,
                                      int32* pnBytesExpected) = 0;

  // Gets metadata for a file after it has been downloaded. This is the same metadata given in
  // the RemoteStorageDownloadUGCResult_t call result
  virtual bool GetUGCDetails(UGCHandle_t hContent, AppId_t* pnAppID,
                             STEAM_OUT_STRING() char** ppchName, int32* pnFileSizeInBytes,
                             STEAM_OUT_STRUCT() CSteamID* pSteamIDOwner) = 0;

  // After download, gets the content of the file.
  // Small files can be read all at once by calling this function with an offset of 0 and
  // cubDataToRead equal to the size of the file. Larger files can be read in chunks to reduce
  // memory usage (since both sides of the IPC client and the game itself must allocate enough
  // memory for each chunk).  Once the last byte is read, the file is implicitly closed and
  // further calls to UGCRead will fail unless UGCDownload is called again. For especially large
  // files (anything over 100MB) it is a requirement that the file is read in chunks.
  virtual int32 UGCRead(UGCHandle_t hContent, void* pvData, int32 cubDataToRead, uint32 cOffset,
                        EUGCReadAction eAction) = 0;

  // Functions to iterate through UGC that has finished downloading but has not yet been read
  // via UGCRead()
  ...
  virtual PublishedFileUpdateHandle_t
               CreatePublishedFileUpdateRequest(PublishedFileId_t unPublishedFileId) = 0;
  ...
  CommitPublishedFileUpdate(PublishedFileUpdateHandle_t updateHandle) = 0;
  // Gets published file details for the given publishedfileid.  If unMaxSecondsOld is greater
  // than 0, cached data may be returned, depending on how long ago it was cached.  A value of 0
  // will force a refresh. A value of k_WorkshopForceLoadPublishedFileDetailsFromCache will use
  // cached data if it exists, no matter how old it is.
  ...
  STEAM_CALL_RESULT(RemoteStorageGetPublishedItemVoteDetailsResult_t)
  virtual SteamAPICall_t
  GetUserPublishedItemVoteDetails(PublishedFileId_t unPublishedFileId) = 0;
  STEAM_CALL_RESULT(RemoteStorageEnumerateUserPublishedFilesResult_t)
  virtual SteamAPICall_t
  ...
  virtual SteamAPICall_t EnumeratePublishedWorkshopFiles(
      EWorkshopEnumerationType eEnumerationType, uint32 unStartIndex, uint32 unCount,
      uint32 unDays, SteamParamStringArray_t* pTags, SteamParamStringArray_t* pUserTags) = 0;
...
STEAM_DEFINE_USER_INTERFACE_ACCESSOR(ISteamRemoteStorage*, SteamRemoteStorage,
...
#if defined(VALVE_CALLBACK_PACK_SMALL)
VALVE_COMPILE_TIME_ASSERT(sizeof(RemoteStorageEnumerateUserSubscribedFilesResult_t) ==
                          (1 + 1 + 1 + 50 + 100) * 4);
#elif defined(VALVE_CALLBACK_PACK_LARGE)
VALVE_COMPILE_TIME_ASSERT(sizeof(RemoteStorageEnumerateUserSubscribedFilesResult_t) ==
                          (1 + 1 + 1 + 50 + 100) * 4 + 4);
