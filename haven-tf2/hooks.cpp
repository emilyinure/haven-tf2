...
void paint(uintptr_t ecx, paint_mode_t mode) {
  ...
  g_cl.m_local = g_interfaces.m_entity_list->get_entity<c_base_player>(
      g_interfaces.m_engine->get_local_player());
...
void lock_cursor(uintptr_t ecx) {
  return g_ui.m_open ? g_interfaces.m_surface->unlock_cursor()
                     : g_hooks.m_original.lock_cursor(ecx);
}
...
void calc_view_model(uintptr_t ecx, c_base_player* owner, const vector& eye_position,
  ...
  vector eye_pos =
      eye_position + ((right * g_ui.m_controls.visuals.view_model.x_offset->m_value) +
...
void c_hooks::init() {
  ...
  MH_CreateHook(g_modules.get("client.dll")
                    .get_sig("40 53 48 83 EC 30 0F 29 74 24 20 49 8B D8")
                    .as<void*>(),
  ...
  MH_CreateHook(c_utils::get_virtual_function<void*>(g_interfaces.m_prediction, 17),
                run_command, reinterpret_cast<void**>(&this->m_original.run_command));

  this->m_original.wnd_proc = reinterpret_cast<WNDPROC>(
      SetWindowLongPtr(g_cl.m_hwnd, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(wnd_proc)));
  MH_CreateHook(c_utils::get_virtual_function<void*>(g_interfaces.m_surface,
                                                     e_indexes::index_lock_cursor),
  ...
  MH_CreateHook(c_utils::get_virtual_function<void*>(g_interfaces.m_client_mode, 16),
                override_view, reinterpret_cast<void**>(&this->m_original.override_view));
  MH_CreateHook(g_modules.get("client.dll")
                    .get_sig("55 8B EC 83 EC 70 8B 55 0C 53 8B 5D 08 89 4D FC 8B 02 89 45 E8 "
                             "8B 42 04 89 45 EC 8B 42 "
                             "08 89 45 F0 56 57")
                    .as<void*>(),
  ...
  MH_CreateHook(g_modules.get("client.dll")
                ...
                reinterpret_cast<void**>(&this->m_original.estimate_abs_velocity));
...
void c_hooks::unload() {
  ...
  MH_Uninitialize();
  SetWindowLongPtr(g_cl.m_hwnd, GWLP_WNDPROC,
                   reinterpret_cast<LONG_PTR>(this->m_original.wnd_proc));
}
