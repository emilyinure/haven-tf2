...
public:
  ...
  STEAM_PRIVATE_API(virtual bool InitGameServer(uint32 unIP, uint16 usGamePort,
                                                uint16 usQueryPort, uint32 unFlags,
                                                AppId_t     nGameAppId,
                                                const char* pchVersionString) = 0;)

  /// Game product identifier.  This is currently used by the master server for version checking
  /// purposes. It's a required field, but will eventually will go away, and the AppID will be
  /// used for this purpose.
  virtual void SetProduct(const char* pszProduct) = 0;

  /// Description of the game.  This is a required field and is displayed in the steam server
  /// browser....for now. This is a required field, but it will go away eventually, as the data
  /// should be determined from the AppID.
  ...
  ///
  /// Note: in previous versions of the SDK, this was automatically called within
  /// SteamGameServer_Init, but this is no longer the case.
  ...
  /// Region identifier.  This is an optional field, the default value is empty, meaning the
  /// "world" region
  ...
  // BeginAuthSession API ). pcbTicket retrieves the length of the actual ticket.
  // SteamNetworkingIdentity is an optional parameter to hold the public IP address of the
  // entity you are connecting to if an IP address is passed Steam will only allow the ticket to
  // be used by an entity with that IP address
  ...
  // Authenticate ticket ( from GetAuthSessionTicket ) from entity steamID to be sure it is
  // valid and isnt reused Registers for callbacks if the entity goes offline or cancels the
  // ticket ( see ValidateAuthTicketResponse_t callback and EAuthSessionResponse )
  ...
  // SendUserConnectAndAuthenticate, use this function to determine if the user owns
  ...
  // it's for us.
  virtual bool HandleIncomingPacket(const void* pData, int cbData, uint32 srcIP,
                                    uint16 srcPort) = 0;

  // AFTER calling HandleIncomingPacket for any packets that came in that frame, call this.
  // This gets a packet that the master server updater needs to send out on UDP.
  // It returns the length of the packet it wants to send, or 0 if there are no more packets to
  // send. Call this each frame until it returns 0.
  ...
  // servers to validate the users identity, app ownership, and VAC status.  If the Steam
  // servers are off-line, then it will validate the cached ticket itself which will validate
  // app ownership and identity.  The AuthBlob here should be acquired on the game client using
  // SteamUser()->InitiateGameConnection() and must then be sent up to the game server for
  // authentication.
  //
  // Return Value: returns true if the users ticket passes basic checks. pSteamIDUser will
  // contain the Steam ID of this user. pSteamIDUser must NOT be NULL If the call succeeds then
  // you should expect a GSClientApprove_t or GSClientDeny_t callback which will tell you
  // whether authentication for the user has succeeded or failed (the steamid in the callback
  ...
  virtual bool SendUserConnectAndAuthenticate_DEPRECATED(uint32      unIPClient,
  ...
  // validation.
  //
  // Return Value: Returns a SteamID for the user to be tracked with, you should call
  // EndAuthSession() when this user leaves the server just like you would for a real user.
  ...
  // currently connected to the server.  For regular users you must call this after you receive
  // a GSUserValidationSuccess callback.
  //
  // Return Value: true if successful, false if failure (ie, steamIDUser wasn't for an active
  // player)
  virtual bool BUpdateUserData(CSteamID steamIDUser, const char* pchPlayerName,
                               uint32 uScore) = 0;
...
STEAM_DEFINE_GAMESERVER_INTERFACE_ACCESSOR(ISteamGameServer*, SteamGameServer,
...
// m_bSecure is true if the game server should display itself as secure to users, false
// otherwise
...
struct GSReputation_t {
  ...
  uint32 m_unBanExpires;   // Time the ban expires, expressed in the Unix epoch (seconds since
                           // 1/1/1970)
};
