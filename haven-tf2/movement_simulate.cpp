...
vector c_movement_simulate::get_player_mins() { return mv.m_player->mins(); }
vector c_movement_simulate::get_player_maxs() { return mv.m_player->maxs(); }
void   c_movement_simulate::trace_player_bbox(const vector& start, const vector& end,
...
void c_movement_simulate::try_touch_ground(c_base_entity* player, const vector& start,
                                           const vector& end, const vector& mins,
                                           const vector& maxs, unsigned int fMask,
...
void c_movement_simulate::try_touch_ground_in_quadrants(
    c_base_entity* player, const vector& start, const vector& end, const vector& minsSrc,
    const vector& maxsSrc, unsigned int fMask, int collisionGroup, trace_t& pm) {
...
int c_movement_simulate::clip_velocity(vector& in, vector& normal, vector& out,
                                       float overbounce) {

  const float angle = normal[2];

  int blocked = 0x00; // Assume unblocked.
  if (angle > 0)     // If the plane that is blocking us has a positive z component, then assume
                     // it's a floor.
    blocked |= 0x01; //
...
int c_movement_simulate::try_player_move() {
  ...
  allFraction = 0;
  time_left   = g_interfaces.m_global_vars
                  ->m_interval_per_tick; // Total time for this movement operation.
  ...
  for (int bumpcount = 0; bumpcount < 4; bumpcount++) {
    ...
    trace_player_bbox(mv.m_position, end, player_solid_mask(), COLLISION_GROUP_PLAYER_MOVEMENT,
                      pm);
    ...
    if (pm.m_fraction > 0) {
      if (pm.m_fraction == 1.f) {
        // There's a precision issue with terrain tracing that can cause a swept box to
        // successfully trace when the end position is stuck in the triangle.  Re-run the test
        // with an uswept box to catch that case until the bug is fixed. If we detect getting
        // stuck, don't allow the movement
        trace_t stuck;
        trace_player_bbox(pm.m_end, pm.m_end, player_solid_mask(),
    ...
    // Only give this a try for first impact plane because you can get yourself stuck in an
    // acute corner by jumping in place
    //  and pressing forward and nobody was really using this bounce/reflection feature
    //  anyway...
...
void c_movement_simulate::friction(void) {
  ...
  if (mv.on_ground) // On an entity that is the ground
  {
    ...
    const float control = (speed < sv_stopspeed->m_value.m_float_value)
                              ? sv_stopspeed->m_value.m_float_value
                              : speed;
...
void c_movement_simulate::categorize_position(void) {
  ...
  // Reset this each time we-recategorize, otherwise we have bogus friction when we jump into
  ...
  try_touch_ground(mv.m_player, vecStartPos, vecEndPos, get_player_mins(), get_player_maxs(),
  ...
  if (pm.m_plane.normal[2] < 0.7f) {
    ...
    // actually stand on
    try_touch_ground_in_quadrants(mv.m_player, vecStartPos, vecEndPos, get_player_mins(),
                                  get_player_maxs(), MASK_PLAYERSOLID,
...
void c_movement_simulate::start_gravity(void) {
  ...
  mv.m_velocity[2] -= (ent_gravity * sv_gravity->m_value.m_float_value * 0.5f *
...
void c_movement_simulate::stay_on_ground(void) {
  ...
  trace_player_bbox(mv.m_position, start, player_solid_mask(), COLLISION_GROUP_PLAYER_MOVEMENT,
                    trace);
  start = trace.m_end;
      ...
      trace.m_plane.normal[2] >= 0.7f) // can't hit a steep slope that we can't stand on anyway
  {
    ...
    // This is incredibly hacky. The real problem is that trace returning that strange value we
    // can't network over.
    if (flDelta > 0.5f * COORD_RESOLUTION) {
...
void c_movement_simulate::air_acceletate(vector& wishdir, const float wishspeed,
                                         const float accel) {
...
void c_movement_simulate::air_move() {
  ...
  vector(0, mv.m_air_eye_dir, 0)
      .angle_vectors(&forward, &right, &up); // Determine movement angles
  float fmove  = 0;
  float smove  = 0;
...
void c_movement_simulate::accelerate(vector& wishdir, const float wishspeed,
  ...
  float accelspeed = accel * g_interfaces.m_global_vars->m_interval_per_tick * wishspeed *
                     mv.m_surface_friction;
...
void c_movement_simulate::step_move(vector target, trace_t& trace) {
  ...
  trace_player_bbox(mv.m_position, end_pos, player_solid_mask(),
  ...
  if ((trace.m_fraction != 1.f && trace.m_plane.normal.m_z < 0.7f) ||
      vector_compare(mv.m_position, pos)) {
    low_road = true;
    up_road  = false;
  }
  ...
  if (low_road) {
    ...
    if (up_road) {
      float up_dist = (up_pos.m_x - pos.m_x) * (up_pos.m_x - pos.m_x) +
                      (up_pos.m_y - pos.m_y) * (up_pos.m_y - pos.m_y);
...
void c_movement_simulate::walk_move() {
  ...
  if (mv.m_walk_direction.length_2d() > 1.f) {
    if (mv.ground_circle_prediction) {
      ...
      float rot =
          (g_interfaces.m_global_vars->m_interval_per_tick * 360.f) / mv.ground_rotation_timing;
  ...
  */
  ...
  // if our wish speed is too low (attributes), we must increase acceleration or we'll never
  // overcome friction Reverse the basic friction calculation to find our required acceleration
  if (wishspeed > 0 && wishspeed < calc_wish_speed_threshold()) {
    ...
    const float fl_control = (fl_speed < sv_stopspeed->m_value.m_float_value)
                                 ? sv_stopspeed->m_value.m_float_value
                                 : fl_speed;
  ...
  if (tf_clamp_back_speed->m_value.m_float_value < 1.0f &&
    ...
    if (flDot < 0) {
      ...
      // Re-run this to prevent crazy values (clients can induce this via usercmd viewangles
      // hacking)
  ...
  dest[0] =
      mv.m_position[0] + mv.m_velocity[0] * g_interfaces.m_global_vars->m_interval_per_tick;
  dest[1] =
      mv.m_position[1] + mv.m_velocity[1] * g_interfaces.m_global_vars->m_interval_per_tick;
  dest[2] = mv.m_position[2];

  // first try moving directly to the next spot
  trace_player_bbox(mv.m_position, dest, player_solid_mask(), COLLISION_GROUP_PLAYER_MOVEMENT,
                    pm);
  ...
  if (!old_ground) {
    // Now pull the base velocity back out.   Base velocity is set if you are on a moving
    // object, like a conveyor (or maybe another monster?)
    mv.m_velocity -= mv.m_base_velocity;
    return;
  }
...
bool c_movement_simulate::charge_move() {
  ...
  mv.m_air_dir = mv.m_walk_direction =
      vector(tf_max_charge_speed->m_value.m_float_value, 0.0f, 0.0f);

  return true;
}

void c_movement_simulate::process_movment() { charge_move(); }
...
float dir_turning1(vector vel, vector last_vel) {
  ...
  float dir = rad_to_deg(std::atan2(vel.m_y, vel.m_x)) -
              rad_to_deg(std::atan2(last_vel.m_y, last_vel.m_x));
  while (dir > 180.f)
    dir -= 360.f;
  while (dir < -180.f)
    dir += 360.f;
...
vector c_movement_simulate::estimate_walking_dir(vector velocity, vector last_fric_vel,
                                                 vector eye_ang, vector origin) {
  if (!grab_convars()) {
    return vector();
  }
...
vector c_movement_simulate::estimate_air_dir(vector velocity, vector last_fric_vel,
                                             vector eye_ang, vector origin) {
  vector forward, right, up;
...
vector c_movement_simulate::find_unstuck(vector origin) {
  trace_t stuck;
  trace_player_bbox(origin, origin, player_solid_mask(), COLLISION_GROUP_PLAYER_MOVEMENT,
                    stuck);
  if (stuck.m_start_solid || stuck.m_fraction != 1.0f) {
    for (auto f = -1; f < 2; f++) {
      for (auto s = -1; s < 2; s++) {
        for (auto z = -1; z < 2; z++) {
          ...
          trace_player_bbox(cur, cur, player_solid_mask(), COLLISION_GROUP_PLAYER_MOVEMENT,
                            stuck);
          if (!stuck.m_start_solid && stuck.m_fraction == 1.0f) {
            return cur;
          }
...
bool c_movement_simulate::grab_convars() {
  if (!sv_gravity || !sv_airaccelerate || !sv_accelerate || !sv_stopspeed || !sv_friction ||
      !tf_clamp_back_speed || !tf_clamp_back_speed_min || !sv_maxvelocity ||
      !tf_max_charge_speed) {
  ...
  return sv_gravity && sv_airaccelerate && sv_accelerate && sv_stopspeed && sv_friction &&
         tf_clamp_back_speed && tf_clamp_back_speed_min && sv_maxvelocity &&
         tf_max_charge_speed;
}
...
void c_movement_simulate::ground_input_prediction(
    const player_t& player_info, const std::shared_ptr<player_record_t>& record) {
  ...
  for (auto& i : m_logs) {
    ...
    for (auto i = log.m_path.end() - 2; i != log.m_path.begin(); --i) {
      ...
      if ((iter % 4) == 0) {
        ...
        right.m_z = 0.f;
        if (math::world_to_screen(last + right * fminf(200, 200 * (delta.length_2d()) / 300.f),
                                  screen_3) &&
