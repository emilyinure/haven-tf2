...
bool c_backstab::check_player( c_base_player* base_player ) {
	auto [m_records, sim_time, pred_origin, player] = g_player_manager.players[ base_player->entindex( ) - 1 ];
	...
	const int lerp = TIME_TO_TICKS(std::fmaxf( cl_interp->m_value.m_float_value, cl_interp_ratio->m_value.m_float_value / cl_updaterate->m_value.m_float_value ));
	...
	if ( record && record->valid( ) ) {
		...
		if ( ( g_cl.m_local->m_class( ) != TF2_Spy || CanPerformBackstabONTarget( look, record->eye_angle, base_player ) ) && DoSwingTraceInternal( look, trace ) ) {
