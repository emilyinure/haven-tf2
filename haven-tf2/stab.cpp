...
bool CanPerformBackstabONTarget(vector& angle, vector& enemy_view, c_base_player* target) {
  vector vecToTarget,
      target_center = ((target->maxs() - target->mins()) * 0.5f) + target->mins(),
      local_center =
          ((g_cl.m_local->maxs() - g_cl.m_local->mins()) * 0.5f) + g_cl.m_local->mins();
  vecToTarget = (target_center + target->get_abs_origin()) -
                (local_center + g_cl.m_local->get_abs_origin());
...
bool DoSwingTraceInternal(vector& angle, trace_t& trace) {
  // Setup a volume for the melee weapon to be swung - approx size, so all melee behave the
  // same.
  static vector vecSwingMinsBase(-18, -18, -18);
  static vector vecSwingMaxsBase(18, 18, 18);

  float fBoundsScale =
      c_attribute_manager::attribute_hook_float(1.0f, "melee_bounds_multiplier", g_cl.m_weapon);
  ...
  // Not scaling down the range for smaller models because midgets need all the help they can
  // get
  ...
  fSwingRange = c_attribute_manager::attribute_hook_float(fSwingRange, "melee_range_multiplier",
                                                          g_cl.m_weapon);
  ...
  trace = trace_t();
  ray.initialize(g_cl.m_shoot_pos, g_cl.m_shoot_pos + vecForward * fSwingRange,
                 vecSwingMinsBase, vecSwingMaxsBase);
  g_interfaces.m_engine_trace->trace_ray(ray, MASK_SOLID, &ignoreTeammatesFilter, &trace);
...
bool c_backstab::check_player(c_base_player* base_player) {
  ...
  if (record && record->valid()) {
    ...
    if (std::powf(fSwingRange, 2) * 2 <=
            (record->origin - g_cl.m_shoot_pos).length_sqr() && // optimization
...
void c_backstab::run() {
  ...
  for (auto i = 1; i < g_interfaces.m_engine->get_max_clients(); i++) {
    ...
    if (!base_player->is_valid(g_cl.m_local,
                               g_cl.m_weapon->item_index() != WPN_DisciplinaryAction))
      continue;
