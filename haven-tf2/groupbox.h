...
public:
  ...
  std::shared_ptr<c_key_bind> add_keybind(const char* name, const char* tooltip,
                                          int default_type = 0);
  std::shared_ptr<c_button>   add_button(const char* name, c_button::callback_fn callback,
                                         const char* tooltip = "");
