...
public:
  ...
  // Create a browser object for display of a html page, when creation is complete the call
  // handle will return a HTML_BrowserReady_t callback for the HHTMLBrowser of your new browser.
  //   The user agent string is a substring to be added to the general user agent string so you
  //   can
  // identify your client on web servers.
  //   The userCSS string lets you apply a CSS style sheet to every displayed page, leave null
  //   if
  ...
  // Call this when you are done with a html surface, this lets us free the resources being used
  // by it
  ...
  // keyboard interactions, native keycode is the virtual key code value from your OS, system
  // key flags the key to not be sent as a typed character as well as a key down
  virtual void KeyDown(HHTMLBrowser unBrowserHandle, uint32 nNativeKeyCode,
                       EHTMLKeyModifiers eHTMLKeyModifiers, bool bIsSystemKey = false) = 0;
  virtual void KeyUp(HHTMLBrowser unBrowserHandle, uint32 nNativeKeyCode,
                     EHTMLKeyModifiers eHTMLKeyModifiers)                              = 0;
  // cUnicodeChar is the unicode character point for this keypress (and potentially multiple
  // chars per press)
  ...
  // tell the html control if it has key focus currently, controls showing the I-beam cursor in
  // text controls amongst other things
  ...
  // find this string in the browser, if bCurrentlyInFind is true then instead cycle to the next
  // matching element
  ...
  // Zoom the current page by flZoom ( from 0.0 to 2.0, so to zoom to 120% use 1.2 ), zooming
  // around point X,Y in the page (use 0,0 if you don't care)
  virtual void SetPageScaleFactor(HHTMLBrowser unBrowserHandle, float flZoom, int nPointX,
                                  int nPointY) = 0;

  // Enable/disable low-resource background mode, where javascript and repaint timers are
  // throttled, resources are more aggressively purged from memory, and audio/video elements are
  // paused. When background mode is enabled, all HTML5 video and audio objects will execute
  // ".pause()" and gain the property "._steam_background_paused = 1". When background mode is
  // disabled, any video or audio objects with that property will resume with ".play()".
  virtual void SetBackgroundMode(HHTMLBrowser unBrowserHandle, bool bBackgroundMode) = 0;

  // Scale the output display space by this factor, this is useful when displaying content on
  // high dpi devices. Specifies the ratio between physical and logical pixels.
...
STEAM_DEFINE_USER_INTERFACE_ACCESSOR(ISteamHTMLSurface*, SteamHTMLSurface,
...
STEAM_CALLBACK_MEMBER(1, const char*, pBGRA) // a pointer to the B8G8R8A8 data for this surface,
...
STEAM_CALLBACK_MEMBER(
    8, uint32, unScrollX) // the page scroll the browser was at when this texture was rendered
STEAM_CALLBACK_MEMBER(
    9, uint32, unScrollY) // the page scroll the browser was at when this texture was rendered
STEAM_CALLBACK_MEMBER(10, float,
                      flPageScale) // the page scale factor on this page when rendered
STEAM_CALLBACK_MEMBER(11, uint32,
...
STEAM_CALLBACK_MEMBER(
    2, const char*, pchTarget) // the html link target type  (i.e _blank, _self, _parent, _top )
STEAM_CALLBACK_MEMBER(3, const char*, pchPostData) // any posted data for the request
STEAM_CALLBACK_MEMBER(
    4, bool, bIsRedirect) // true if this was a http/html redirect from the last load request
STEAM_CALLBACK_END(5)

//-----------------------------------------------------------------------------
// Purpose: The browser has been requested to close due to user interaction (usually from a
// javascript window.close() call)
//-----------------------------------------------------------------------------
...
STEAM_CALLBACK_MEMBER(
    3, bool, bIsRedirect) // true if this was a http/html redirect from the last load request
STEAM_CALLBACK_MEMBER(4, const char*, pchPageTitle) // the title of the page
STEAM_CALLBACK_MEMBER(
    5, bool,
    bNewNavigation) // true if this was from a fresh tab and not a click on an existing page
...
STEAM_CALLBACK_MEMBER(0, HHTMLBrowser,
                      unBrowserHandle) // this is the new browser handle after the restart
STEAM_CALLBACK_MEMBER(
    1, HHTMLBrowser,
    unOldBrowserHandle) // the handle for the browser before the restart, if your handle was
                        // this then switch to using unBrowserHandle for API calls
STEAM_CALLBACK_END(2)
