...
enum EGCResults {
  ...
  k_EGCResultInvalidMessage =
      4, // Something was wrong with the message being sent with SendMessage
};
...
public:
  ...
  // returns k_EGCResultNoMessage if there is no message waiting. pcubMsgSize is filled with the
  // message size. If the provided buffer is not large enough to fit the entire message,
  // k_EGCResultBufferTooSmall is returned and the message remains at the head of the queue.
