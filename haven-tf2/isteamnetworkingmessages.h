...
public:
  ...
  /// get through, an error will be posted via the callback
  /// SteamNetworkingMessagesSessionFailed_t. There is no notification when the operation
  /// succeeds.  (You should have the peer send a reply for this purpose.)
  ///
  /// Sending a message to a host will also implicitly accept any incoming connection from that
  /// host.
  ///
  ...
  /// You'll have to call ReceiveMessagesOnChannel() with the same channel number in order to
  ...
  virtual EResult SendMessageToUser(const SteamNetworkingIdentity& identityRemote,
                                    const void* pubData, uint32 cubData, int nSendFlags,
                                    int nRemoteChannel) = 0;

  /// Reads the next message that has been sent from another user via SendMessageToUser() on the
  /// given channel. Returns number of messages returned into your list.  (0 if no message are
  /// available on that channel.)
  ///
  /// When you're done with the message object(s), make sure and call
  /// SteamNetworkingMessage_t::Release()!
  virtual int ReceiveMessagesOnChannel(int                        nLocalChannel,
                                       SteamNetworkingMessage_t** ppOutMessages,
                                       int                        nMaxMessages) = 0;

  /// Call this in response to a SteamNetworkingMessagesSessionRequest_t callback.
  /// SteamNetworkingMessagesSessionRequest_t are posted when a user tries to send you a
  /// message, and you haven't tried to talk to them first.  If you don't want to talk to them,
  /// just ignore the request.  If the user continues to send you messages,
  /// SteamNetworkingMessagesSessionRequest_t callbacks will continue to be posted periodically.
  ///
  ...
  ///
  /// Calling SendMessageToUser() will implicitly accepts any pending session request to that
  /// user.
  virtual bool AcceptSessionWithUser(const SteamNetworkingIdentity& identityRemote) = 0;

  /// Call this when you're done talking to a user to immediately free up resources
  /// under-the-hood. If the remote user tries to send data to you again, another
  /// SteamNetworkingMessagesSessionRequest_t callback will be posted.
  ///
  ...
  virtual bool CloseChannelWithUser(const SteamNetworkingIdentity& identityRemote,
                                    int                            nLocalChannel) = 0;
  ...
  /// failure information.  (See SendMessageToUser and
  /// k_nSteamNetworkingSend_AutoRestartBrokenSession.)
  ///
  /// Returns the value of SteamNetConnectionInfo_t::m_eState, or
  /// k_ESteamNetworkingConnectionState_None if no connection exists with specified peer.  You
  /// may pass nullptr for either parameter if you do not need the corresponding details.  Note
  /// that sessions time out after a while, so if a connection fails, or SendMessageToUser
  /// returns k_EResultNoConnection, you cannot wait indefinitely to obtain the reason for
  /// failure.
  virtual ESteamNetworkingConnectionState
  GetSessionConnectionInfo(const SteamNetworkingIdentity&      identityRemote,
...
/// Posted when a remote host is sending us a message, and we do not already have a session with
/// them
...
// Steamworks SDK
STEAM_DEFINE_USER_INTERFACE_ACCESSOR(ISteamNetworkingMessages*,
                                     SteamNetworkingMessages_SteamAPI,
