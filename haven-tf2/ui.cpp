...
void c_ui::init() {
  ...
  const auto main_window = this->add_window("haven", {290, 50, 548, 350});
  {
    const auto aim = main_window->add_tab("Aim");
    {
      const auto players = aim->add_groupbox("Projectile", {0, 5, 234, 322});
      {
        ...
        this->m_controls.aim.players.interp =
            players->add_slider("Interp", 4, 10, "Interpolation On Player Prediction");
    ...
    const auto visuals = main_window->add_tab("Visuals");
    {
      ...
      {
        this->m_controls.visuals.view_model.x_offset =
            viewmodel->add_slider("X Offset", -10, 10);
        this->m_controls.visuals.view_model.y_offset =
            viewmodel->add_slider("Y Offset", -10, 10);
        this->m_controls.visuals.view_model.z_offset =
            viewmodel->add_slider("Z Offset", -10, 10);
    ...
    const auto misc = main_window->add_tab("Misc");
    {
      const auto misc2 = misc->add_groupbox("Misc", {0, 5, 473, 322});
      {
        ...
        this->m_controls.misc.third_person_horizontal_offset =
            misc2->add_slider("Thirdperson Horizontal Offset", -100, 100);
        this->m_controls.misc.third_person_vertical_offset =
            misc2->add_slider("Thirdperson Vertical Offset", -100, 100);
        this->m_controls.misc.third_person_distance_offset =
            misc2->add_slider("Thirdperson Distance Offset", -100, 100);
      }
    }
  }
  ...
  const auto config_window = this->add_window("Config", {15, 50, 270, 128});
  {
    const auto tab = config_window->add_tab("Main");
    {
      const auto groupbox = tab->add_groupbox("Config", {0, 5, 191, 91});
      {
        ...
        this->m_controls.config.save = groupbox->add_button(
            "Save", save_callback, "Save your config (located in base tf2 folder)");
        this->m_controls.config.load = groupbox->add_button(
            "Load", load_callback, "Load your config (located in base tf2 folder)");
      }
    }
  }
  g_config.init();
...
void c_ui::on_paint() {
  ...
  std::ranges::sort(this->m_windows, [&](const std::shared_ptr<c_window>& first,
                                         const std::shared_ptr<c_window>& second) {
...
void c_ui::draw_cursor() {
  g_ui.m_draw.filled_rect({g_input.m_mouse_pos.m_x - 4, g_input.m_mouse_pos.m_y - 4, 8, 8},
                          {70, 70, 70});
  g_ui.m_draw.outlined_rect({g_input.m_mouse_pos.m_x - 4, g_input.m_mouse_pos.m_y - 4, 8, 8},
                            {85, 85, 85});
}
