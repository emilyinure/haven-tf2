...
void key_bind_t::update( ) {
	...
	case key_bind_type_always: enabled = true;
		break;
	case key_bind_type_hold: enabled = g_input.key_down( key );
		break;
	...
	const auto main_window = this->add_window( "haven", { 290, 50, 300, 350 } ); {
		const auto aim = main_window->add_tab( "Aim" ); {
			const auto players = aim->add_groupbox( "Players", { 0, 5, 215, 250 } ); {
				...
				this->m_controls.aim.players.interp = players->add_slider( "Interp", 1, 10, "Interpolation On Player Prediction" );
				this->m_controls.aim.players.hitchance = players->add_slider( "Hitchance", 0, 100, "Hitchance For Player Prediction" );
	...
	const auto config_window = this->add_window( "Config", { 15, 50, 270, 128 } ); {
		const auto tab = config_window->add_tab( "Main" ); {
			const auto groupbox = tab->add_groupbox( "Config", { 0, 5, 191, 91 } ); {
				...
				this->m_controls.config.save = groupbox->add_button( "Save", save_callback, "Save your config (located in base tf2 folder)" );
				this->m_controls.config.load = groupbox->add_button( "Load", load_callback, "Load your config (located in base tf2 folder)" );
			}
		}
	}
	g_config.init( );
...
void c_ui::on_paint( ) {
	...
	std::ranges::sort( this->m_windows, [ & ](  const std::shared_ptr< c_window >& first, const std::shared_ptr< c_window >& second ) {
