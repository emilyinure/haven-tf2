...
public:
  ...
  virtual float
  GetLatency(int flow) const = 0; // current latency (RTT), more accurate but jittering
  ...
  virtual float
  GetTimeSinceLastReceived(void) const = 0; // get time since last recieved packet in seconds
...
public:
  ...
  virtual bool SendNetMsg(INetMessage& msg, bool bForceReliable = false,
                          bool bVoice = false) = 0;
#ifdef POSIX
  FORCEINLINE bool SendNetMsg(INetMessage const& msg, bool bForceReliable = false,
                              bool bVoice = false) {
  ...
  virtual float
  GetTimeSinceLastReceived(void) const = 0; // get time since last received packet in seconds
...
public:
  ...
  virtual const char*
  ToString(void) const = 0; // returns a human readable string about message content
};
class INetChannelHandler {
public:
  ...
  virtual void
  ConnectionStart(INetChannel* chan) = 0; // called first time network channel is established

  virtual void
  ConnectionClosing(const char* reason) = 0; // network channel is being closed by remote site

  virtual void ConnectionCrashed(const char* reason) = 0; // network error occured

  virtual void
  PacketStart(int incoming_sequence,
  ...
  virtual void
  FileRequested(const char*  fileName,
  ...
  virtual void
  FileDenied(const char*  fileName,
...
public:
  ...
  // CUtlVector<dataFragments_t *> m_WaitingList[MAX_STREAMS]; // waiting list for reliable data
  // and file transfer
  ...
  int  m_StreamReceived; // length of already received BYTEs
                         // char m_SteamFile[MAX_OSPATH];  // if receiving file, this is it's
                         // name CUtlMemory<BYTE> m_StreamData; // Here goes the stream data (if
                         // not file). Only allocated if we're going to use it.
  ...
  INetChannelHandler*
      m_MessageHandler; // who registers and processes messages
                        // CUtlVector<INetMessage *> m_NetMessages; // list of registered
                        // message IDemoRecorder *m_DemoRecorder;			 // if
                        // != NULL points to a recording/playback demo object
...
//	virtual void get_packet_response_latency( int flow, int frame_number, int*
// latency_msecs, int* choke ) const = 0; 	virtual void get_remote_framerate( float*
// frame_time, float* frame_time_std_deviation ) const = 0; 	virtual float
// get_timeout_seconds( ) const = 0;
// };
