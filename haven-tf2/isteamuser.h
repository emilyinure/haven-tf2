...
public:
  ...
  // this is only used internally by the API, and by a few select interfaces that support
  // multi-user
  virtual HSteamUser GetHSteamUser() = 0;

  // returns true if the Steam client current has a live connection to the Steam servers.
  // If false, it means there is no active connection due to either a networking issue on the
  // local machine, or the Steam server is down/busy. The Steam client will automatically be
  // trying to recreate the connection as often as possible.
  virtual bool BLoggedOn() = 0;

  // returns the CSteamID of the account currently logged into the Steam client
  // a CSteamID is a unique identifier for an account, and used to differentiate users in all
  // parts of the Steamworks API
  virtual CSteamID GetSteamID() = 0;

  // Multiplayer Authentication functions

  // InitiateGameConnection() starts the state machine for authenticating the game client with
  // the game server It is the client portion of a three-way handshake between the client, the
  // game server, and the steam servers
  //
  // Parameters:
  // void *pAuthBlob - a pointer to empty memory that will be filled in with the authentication
  // token. int cbMaxAuthBlob - the number of bytes of allocated memory in pBlob. Should be at
  // least 2048 bytes. CSteamID steamIDGameServer - the steamID of the game server, received
  // from the game server by the client CGameID gameID - the ID of the current game. For games
  // without mods, this is just CGameID( <appID> ) uint32 unIPServer, uint16 usPortServer - the
  // IP address of the game server bool bSecure - whether or not the client thinks that the game
  // server is reporting itself as secure (i.e. VAC is running)
  //
  // return value - returns the number of bytes written to pBlob. If the return is 0, then the
  // buffer passed in was too small, and the call has failed The contents of pBlob should then
  // be sent to the game server, for it to use to complete the authentication process.
  //
  ...
  virtual int InitiateGameConnection_DEPRECATED(void* pAuthBlob, int cbMaxAuthBlob,
                                                CSteamID steamIDGameServer, uint32 unIPServer,
                                                uint16 usPortServer, bool bSecure) = 0;

  // notify of disconnect
  // needs to occur when the game client leaves the specified game server, needs to match with
  // the InitiateGameConnection() call
  //
  // DEPRECATED!  This function will be removed from the SDK in an upcoming version.
  //              Please migrate to BeginAuthSession and related functions.
  ...
  // get the local storage folder for current Steam account to write application data, e.g. save
  // games, configs etc. this will usually be something like "C:\Progam
  // Files\Steam\userdata\<SteamID>\<AppID>\local"
  virtual bool GetUserDataFolder(char* pchBuffer, int cubBuffer) = 0;

  // Starts voice recording. Once started, use GetVoice() to get the data
  virtual void StartVoiceRecording() = 0;

  // Stops voice recording. Because people often release push-to-talk keys early, the system
  // will keep recording for a little bit after this function is called. GetVoice() should
  // continue to be called until it returns k_eVoiceResultNotRecording
  ...
  GetAvailableVoice(uint32* pcbCompressed, uint32* pcbUncompressed_Deprecated = 0,
  ...
  virtual EVoiceResult GetVoice(bool bWantCompressed, void* pDestBuffer,
                                ...
                                bool    bWantUncompressed_Deprecated                   = false,
  ...
  virtual EVoiceResult DecompressVoice(const void* pCompressed, uint32 cbCompressed,
                                       ...
                                       uint32* nBytesWritten, uint32 nDesiredSampleRate) = 0;
  ...
  // SteamNetworkingIdentity is an optional input parameter to hold the public IP address or
  // SteamID of the entity you are connecting to if an IP address is passed Steam will only
  // allow the ticket to be used by an entity with that IP address if a Steam ID is passed Steam
  // will only allow the ticket to be used by that Steam ID not to be used for
  // "ISteamUserAuth\AuthenticateUserTicket" - it will fail
  virtual HAuthTicket
  GetAuthSessionTicket(void* pTicket, int cbMaxTicket, uint32* pcbTicket,
  ...
  // pchIdentity is an optional input parameter to identify the service the ticket will be sent
  // to the ticket will be returned in callback GetTicketForWebApiResponse_t
  virtual HAuthTicket GetAuthTicketForWebApi(const char* pchIdentity) = 0;

  // Authenticate ticket from entity steamID to be sure it is valid and isnt reused
  // Registers for callbacks if the entity goes offline or cancels the ticket ( see
  // ValidateAuthTicketResponse_t callback and EAuthSessionResponse )
  virtual EBeginAuthSessionResult BeginAuthSession(const void* pAuthTicket, int cbAuthTicket,
                                                   CSteamID steamID) = 0;

  // Stop tracking started by BeginAuthSession - called when no longer playing game with this
  // entity
  virtual void EndAuthSession(CSteamID steamID) = 0;

  // Cancel auth ticket from GetAuthSessionTicket, called when no longer playing game with the
  // entity you gave the ticket to
  virtual void CancelAuthTicket(HAuthTicket hAuthTicket) = 0;

  // After receiving a user's authentication data, and passing it to BeginAuthSession, use this
  // function to determine if the user owns downloadable content specified by the provided
  // AppID.
  virtual EUserHasLicenseForAppResult UserHasLicenseForApp(CSteamID steamID, AppId_t appID) = 0;

  // returns true if this users looks like they are behind a NAT device. Only valid once the
  // user has connected to steam (i.e a SteamServersConnected_t has been issued) and may not
  // catch all forms of NAT.
  virtual bool BIsBehindNAT() = 0;

  // set data to be replicated to friends so that they can join your game
  // CSteamID steamIDGameServer - the steamID of the game server, received from the game server
  // by the client uint32 unIPServer, uint16 usPortServer - the IP address of the game server
  ...
  // the user has can have two different badges for a series; the regular (max level 5) and the
  // foil (max level 1)
...
//			This means the Steam client now has a working connection to the Steam
// servers. 			Usually this will have occurred before the game has launched,
// and should 			only be seen if the user has dropped connection due to a
...
//-----------------------------------------------------------------------------
// Purpose: Sent by the Steam server to the client telling it to disconnect from the specified
// game server,
//			which it may be in the process of or already connected to.
//			The game client should immediately disconnect upon receiving this
// message. 			This can usually occur if the user doesn't have rights to play
// on the game server.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: called when the callback system for this client is in an error state (and has
// flushed pending callbacks)
//			When getting this message the client should disconnect from Steam, reset
// any stored Steam state and reconnect. 			This usually occurs in the rare
...
struct MarketEligibilityResponse_t {
  ...
  int m_cdaySteamGuardRequiredDays; // The number of days any user is required to have had Steam
                                    // Guard before they can use the market
  int m_cdayNewDeviceCooldown; // The number of days after initial device authorization a user
                               // must wait before using the market on that device
};
...
struct DurationControl_t {
  ...
  EResult m_eResult; // result of call (always k_EResultOK for asynchronous timer-based
                     // notifications)
  AppId_t m_appid;   // appid generating playtime

  bool  m_bApplicable; // is duration control applicable to user + game combination
  int32 m_csecsLast5h; // playtime since most recent 5 hour gap in playtime, only counting up to
                       // regulatory limit of playtime, in seconds

  EDurationControlProgress
      m_progress; // recommended progress (either everything is fine, or please exit game)
  EDurationControlNotification
      m_notification; // notification to show, if any (always
                      // k_EDurationControlNotification_None for API calls)
